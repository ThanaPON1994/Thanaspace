-- ATLAS v8.8 — Copy Hop + Copy Target + Paste Gemini + Auto Navigate
-- 🐷 Pink Pig Edition — No-Reject + Fuzzy Match Edition
-- 🆕 ปล่อยผ่านทุกคำ EN ที่ไม่ใช่ UI junk
-- 🆕 4-level fuzzy: exact → ci → super → word → substring
-- 🆕 Goal auto-fire ตอน HOP + 777
...
-- ==================== 🆕 FUZZY MATCH ====================
-- ค้นหา candidate ที่ match กับ pick (ไล่ระดับ)
local function findFuzzyMatch(pick, candidates)
    ...
    -- 2. super-fuzzy (ตัด punctuation/diacritics)
    if #pickSuper >= 2 then
        for _, c in ipairs(candidates) do
    ...
    -- 3. word-match (คำทุกคำของ pick ต้องมีใน candidate)
    local pickWords = {}
...
local function doGo()
    ...
    -- ใช้ลิสต์จาก HOP ถ้าหน้าไม่เปลี่ยน
    ...
    for _, p in ipairs(data.picks) do
        ...
        elseif candidateSet[t:lower()] then
            ...
            if match then
                ...
                -- 🆕 ปล่อยผ่านทั้งหมด (ให้ navigateToTitle จัดการ)
                addLine("[allow] " .. t .. " → ลอง navigate ตรง", ORANGE_WARN)
                table.insert(validPicks, p)
            end
        end
    end
...
addLine("🐷 v8.8 No-Reject + Fuzzy", GREEN_OK)
addLine("小女孩 → เช็ค Goal → Fire 🔥", PINK_DEEP)
addLine("Cap = " .. currentCap .. " (แก้ได้ 1-1000)", PINK_DARK)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v8.8 🐷 No-Reject + Fuzzy] Loaded ✓")
