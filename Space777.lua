-- ATLAS v9.7 — Groq 9-Keys + Compound + Rank1 + Thai Prompt + Copy Log
-- 🐷 Pink Pig Edition
...
local CFG = {max_cand = 40, temp = 0.2, max_tok = 100}
...
local function parseAny(text, cands)
    ...
    -- fallback: หาชื่อใน cands ที่ตรงกับ text
    if cands then
        for _, c in ipairs(cands) do
...
local function buildPrompt(cands)
    ...
    return string.format([[คุณคือผู้ช่วยเล่นเกม Wikipedia Speedrun มีหน้าที่เลือกหน้าถัดไปเพื่อไปถึง TARGET โดยใช้จำนวนก้าวให้น้อยที่สุด

กฎการเลือก:
1. ใช้ CURRENT, TARGET และ CANDIDATES เป็นข้อมูลหลัก ไม่จำเป็นต้องมีคำใบ้
2. ถ้า TARGET อยู่ใน CANDIDATES ให้เลือกทันที
3. เลือกได้เฉพาะรายการใน CANDIDATES โดยคัดลอกหมายเลขและชื่อให้ตรง ห้ามสร้างตัวเลือกเอง
4. หากมีข้อมูลยืนยันเส้นทางในเกม ให้ให้ความสำคัญเหนือความเกี่ยวข้องทางความหมาย
5. หากไม่มีเส้นทางยืนยัน ให้คาดการณ์ตัวเลือกที่น่าจะพาเข้าใกล้ TARGET มากที่สุด โดยใช้ความรู้ทั่วไปช่วยประเมิน แต่ห้ามถือว่าเส้นทางที่คาดการณ์มีอยู่จริง
6. พิจารณาว่าหน้าถัดไปน่าจะเชื่อมต่อไปยังอะไร ไม่เลือกเพียงเพราะชื่อคล้ายหรือเกี่ยวข้องกับ TARGET
7. หลีกเลี่ยงการย้อนกลับหรือวนหน้าที่ปรากฏในประวัติที่ได้รับ หากไม่มีประวัติ ห้ามสมมติขึ้นเอง
8. ใช้หน้าศูนย์กลางเมื่อช่วยเชื่อมไปยังเป้าหมาย ไม่เลือกหน้ากว้างโดยไม่จำเป็น
9. ไม่จำกัดการข้ามประเทศ หมวดหมู่ หรือประเภทหัวข้อ หากเป็นทางเลือกที่เหมาะสมกว่า
10. เมื่อมีตัวเลือกครบ ให้เลือกหนึ่งรายการทันที แม้ยังยืนยันเส้นทางทั้งหมดไม่ได้
11. หากขาด CURRENT, TARGET หรือ CANDIDATES ให้ขอเฉพาะข้อมูลที่ขาดในหนึ่งบรรทัด ห้ามเดาหมายเลขหรือตัวเลือก
12. เมื่อเลือกได้ ให้ตอบข้อความธรรมดาเพียงหนึ่งบรรทัด ไม่มีคำอธิบาย JSON หรือข้อความอื่น:
เลือก: [หมายเลข]. [ชื่อ candidate]

ข้อมูลที่ส่งในแต่ละรอบ:
CURRENT: %s
TARGET: %s
CANDIDATES:
%s

เลือก:]], cArt or "?", tTitle or "?", table.concat(lines, "\n"))
end
...
updM()
addL("🐷 v9.7 ready", G)
addL("9 keys | compound only | Thai Prompt", PD)
addL("小女孩 | 🤖 | ▶ AUTO", PD)
sL.Text = "🐽 Ready."
updLbl()
print("[ATLAS v9.7 🐷 Thai Prompt] Loaded ✓")
