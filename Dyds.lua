-- ATLAS v9.4.3 — Copy Hop + Copy Target + Paste Gemini + Auto Navigate (FULL FIX + FAST 777)
-- 🐷 Pink Pig Edition — Cap hidden 250 + pick_index + confirmation + No ID Log
...
-- 🆕 Cap คงที่ 250 (ซ่อนใน Script ไม่แสดงใน GUI)
...
local function doGo()
    ...
    -- 🆕 ลบการแจ้งเตือน "AI ไม่ยืนยัน" ออก
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    table.insert(lines, "2. ห้ามระบุชื่อ 'Target' หรือชื่อเฉพาะของปลายทางลงในคำตอบ ให้ใช้คำว่า 'เป้าหมาย' แทน")
    table.insert(lines, "3. ต้องระบุ 'pick_index' เป็นหมายเลข (1-" .. #filtered .. ") ที่ตรงกับลำดับใน CANDIDATES ที่คุณเลือกเสมอ")
    table.insert(lines, "4. ต้องระบุ 'confirmation' ยืนยันว่าเลือกหมายเลขอะไรจากทั้งหมดกี่คำ")
    table.insert(lines, "5. หากเลือกคำที่ไม่อยู่ใน CANDIDATES ให้ระบุ 'pick_index': 'OUT_OF_LIST' (แต่ควรหลีกเลี่ยง)")
    table.insert(lines, "6. ห้ามตอบเป็น JSON หรือใส่เครื่องหมาย {} เด็ดขาด")
    table.insert(lines, "7. ห้ามพิมพ์ข้อความทักทาย ห้ามเกริ่นนำ ห้ามอธิบายเพิ่มเติม")
    table.insert(lines, "8. ห้ามใส่เครื่องหมาย ** (bold) หรือ ` (backtick) ในคำตอบเด็ดขาด")
    table.insert(lines, "--------------------------------------------------")
    table.insert(lines, "")
    table.insert(lines, "📋 รูปแบบการตอบกลับที่ต้องทำตาม (ห้ามเบี่ยงเบน):")
    table.insert(lines, "ให้ตอบเป็นข้อความธรรมดา 2 บรรทัดเท่านั้น:")
    table.insert(lines, "")
    table.insert(lines, "บรรทัดที่ 1: ฉันเลือกรายการ [X] จาก [Y]")
    table.insert(lines, "บรรทัดที่ 2: คำตอบ: [ชื่อคำที่เลือก]")
    table.insert(lines, "")
    table.insert(lines, "ตัวอย่างการตอบที่ถูกต้อง:")
    table.insert(lines, "ฉันเลือกรายการ 13 จาก " .. #filtered)
    table.insert(lines, "คำตอบ: ESPN")
    table.insert(lines, "--------------------------------------------------")
...
pasteBtn.MouseButton1Click:Connect(function()
    -- 🆕 ลบ isProcessing ออก → กดรัวๆ ได้เร็วขึ้น
...
copyLogBtn.MouseButton1Click:Connect(function()
    ...
    table.insert(lines, "2. ห้ามระบุชื่อ 'Target' หรือชื่อเฉพาะของปลายทางลงในคำตอบ ให้ใช้คำว่า 'เป้าหมาย' แทน")
    table.insert(lines, "3. ห้ามตอบเป็น JSON หรือใส่เครื่องหมาย {} เด็ดขาด")
    table.insert(lines, "4. ห้ามพิมพ์ข้อความทักทาย ห้ามเกริ่นนำ ห้ามอธิบายเพิ่มเติม")
    table.insert(lines, "5. ห้ามใส่เครื่องหมาย ** (bold) หรือ ` (backtick) ในคำตอบเด็ดขาด")
    table.insert(lines, "--------------------------------------------------")
    table.insert(lines, "")
    table.insert(lines, "📋 รูปแบบการตอบกลับที่ต้องทำตาม (ห้ามเบี่ยงเบน):")
    table.insert(lines, "ให้ตอบเป็นข้อความธรรมดา 2 บรรทัดเท่านั้น:")
    ...
    table.insert(lines, "")
    table.insert(lines, "ตัวอย่างการตอบที่ถูกต้อง:")
    table.insert(lines, "ฉันเลือกรายการ 1 จาก " .. #lastFallbackCandidates)
...
addLine("🐷 v9.4.3 pick_index + confirmation (FULL FIX + FAST 777)", GREEN_OK)
addLine("小女孩 → Copy CURRENT+Target+CANDIDATES (2 บรรทัด)", PINK_DEEP)
addLine("COPY LOG → Copy No ID Fallback (2 บรรทัด)", ORANGE_WARN)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v9.4.3 🐷 Cap hidden 250 FULL FIX + FAST 777] Loaded ✓")
