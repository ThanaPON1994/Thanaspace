-- ATLAS v8.4 — Copy Hop + Copy Target + Paste Gemini + Auto Navigate
-- 🐷 Pink Pig Edition (350 x 300) — FULL FIX v7
...
-- 🆕 🆕 🆕 SCAN ปุ่ม ? POPUP (ทุกรูปแบบ/ทุกสี)
...
-- ==================== 🆕 GRAY / BLACK DETECTION ====================
local function isGrayOrBlack(color)
    if not color then return false end
    local r, g, b = color.R * 255, color.G * 255, color.B * 255
    
    -- ต้องเป็น greyscale (r ≈ g ≈ b)
    local maxDiff = math.max(math.abs(r - g), math.abs(g - b), math.abs(r - b))
    if maxDiff > 25 then return false end
    
    -- ครอบคลุมทุกโทน: ดำ 0 → เทาอ่อนมาก 230
    return true
end
...
-- ==================== 🆕 SCAN HINT FROM ? BUTTON (Aggressive) ====================
local function scanHintFromPopup()
    if not targetTitle or targetTitle == "" then return "" end
    ...
    for _, obj in ipairs(PlayerGui:GetDescendants()) do
        if not obj:IsDescendantOf(gui) then
            ...
            if obj:IsA("TextButton") or obj:IsA("TextLabel") then
                local text = (obj.Text or ""):gsub("%s", "")
                if text == "?" or text == "？" or text == "❓" or text == "❔" or text == "⍰" then
                    score = score + 100
                end
                if (obj.Name or ""):find("%?") then score = score + 50 end
            end
            
            -- 1b. Image มี ? / question / help
            if obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
                local img = (obj.Image or ""):lower()
                if img:find("question") or img:find("help") 
                   or img:find("hint") or img:find("info")
                   or img:find("tooltip") or img:find("query")
                   or img:find("%?") then
                    score = score + 80
                end
            end
            ...
            if objName:find("question") or objName:find("help")
               or objName:find("hint") or objName:find("info")
               or objName:find("tooltip") or objName:find("tool") then
                score = score + 60
            end
            ...
            if corner then
                ...
                if cr and cr.Scale >= 0.5 then
                    score = score + 30
                end
            end
            
            -- 1e. ขนาดเล็ก ≤60x60 ทรงกลม
            local size = obj.AbsoluteSize
            if size and size.X >= 10 and size.X <= 60
               and size.Y >= 10 and size.Y <= 60 
               and math.abs(size.X - size.Y) <= 10 then
                score = score + 20
            end
            ...
            if obj:IsA("GuiObject") then
                ...
                if isGrayOrBlack(bg) then score = score + 10 end
                if tc and isGrayOrBlack(tc) then score = score + 20 end
            end
            
            -- 1h. Position ด้านบน
            local pos = obj.AbsolutePosition
            if pos and pos.Y < 200 then
                score = score + 5
            end
    ...
    if #candidates == 0 then
        addLine("[?] ไม่เจอปุ่ม ?", PINK_DEEP)
        return ""
    end
    
    -- ═══════════════════════════════════════
    -- 2. ลองคลิกทีละตัว (สูงสุด 5)
    -- ═══════════════════════════════════════
    for i = 1, math.min(5, #candidates) do
        local btn = candidates[i].obj
        local score = candidates[i].score
        
        -- หา Button แม่ (ถ้าเป็น Label)
        local target = btn
        if btn:IsA("TextLabel") or btn:IsA("ImageLabel") then
            local p = btn.Parent
            while p and p ~= PlayerGui do
        ...
        addLine("[?] ลองคลิก #" .. i .. " (score=" .. score .. ")", ORANGE_WARN)
        ...
        pcall(function()
            if target.MouseButton1Click then target.MouseButton1Click:Fire() end
        end)
        pcall(function()
                ...
                if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                    ...
                    if #text >= 30 then
                        ...
                        if vis then
                            ...
                            local hasTarget = textLow:find(targetLow, 1, true)
                            
                            -- ลอง 2 คำแรก
                            if not hasTarget then
                                local words = {}
                                for w in targetLow:gmatch("%a+") do
                                    if #w >= 3 then
                                ...
                                if #words >= 2 then
                                    ...
                                    for _, w in ipairs(words) do
                                        if textLow:find(w, 1, true) then m = m + 1 end
                                    end
                                    if m >= 2 then hasTarget = true end
                                end
                            end
                            
                            -- popup ยาวก็เอา
                            if hasTarget or #text >= 100 then
                                local cleanText = text:gsub("<[^>]+>", "")
                                cleanText = cleanText:gsub("^%s+",""):gsub("%s+$",""):gsub("%s+", " ")
                                
                                local sc = #cleanText
                                if hasTarget then sc = sc + 1000 end
                                if textLow:sub(1, #targetLow) == targetLow then
                                    sc = sc + 2000
                                end
        ...
        addLine("[?] popup +" .. newCount .. " objects", PINK_DARK)
        ...
        -- ถ้าได้ hint → return เลย
        if bestHint ~= "" then
            if #bestHint > 400 then
                bestHint = bestHint:sub(1, 397) .. "..."
            end
            addLine("[?] ได้ hint " .. #bestHint .. " chars", GREEN_OK)
            return bestHint
        end
    end
    
    addLine("[?] popup ไม่มี Target", PINK_DEEP)
    return ""
end
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    -- 🆕 ลอง scanHint ปกติก่อน
    local hint = scanHint()

    -- 🆕 ถ้าไม่เจอ → ลองกดปุ่ม ? เพื่อเปิด popup
    if hint == "" then
        addLine("[hint] ไม่เจอบนจอ → ลองปุ่ม ?", ORANGE_WARN)
        hint = scanHintFromPopup()
    end
    ...
    if hint ~= "" then
        ...
        addLine("[hint] ไม่เจอเลย", PINK_DEEP)
    end
