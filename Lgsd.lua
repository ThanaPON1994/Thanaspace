...
-- 🐷 Pink Pig Edition — Cap Input Edition
-- 🆕 Cap input 1-1000, default 60
-- 🆕 Copy format: "เลือก คำ 1-1000 เท่านั้น..."
...
hopBtn.Size = UDim2.new(1, -120, 0, 28)   -- 🆕 ย่อเพื่อวาง capBox ขวา
...
local function getFilteredCandidates()
    ...
    -- 🆕 ใช้ currentCap แทน hardcode 60
...
local function doGo()
    ...
    if targetTitle and targetTitle ~= "" then
        ...
        for line in text:gmatch("[^\r\n]+") do
            ...
            if lineLow:find(targetLow, 1, true) then
                addLine("[warn] Gemini บอก Goal: " .. line:sub(1, 40), RED_ERR)
    ...
    -- กรอง picks นอกลิสต์
    local currentCandidates = getFilteredCandidates()
    local candidateSet = {}
    ...
    for _, p in ipairs(data.picks) do
        ...
        elseif candidateSet[t:lower()] then
            ...
            local found = false
            for _, c in ipairs(currentCandidates) do
                if superNormalize(c) == superNormalize(t) then
                    p.title = c
                    table.insert(validPicks, p)
                    addLine("[fix] " .. t .. " → " .. c, ORANGE_WARN)
                    found = true
                    break
                end
            end
            if not found then
                addLine("[reject] นอกลิสต์: " .. t, RED_ERR)
            end
        end
    end

    if #validPicks == 0 then
        addLine("[X] Gemini แนะนอกลิสต์ — ไม่มี pick ที่ใช้ได้", RED_ERR)
        statusLbl.Text = "Invalid picks"
        return
    end
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    -- 🆕 Format ใหม่: "เลือก คำ 1-1000 เท่านั้น..."
    local lines = {
        "เลือก คำ 1-1000 เท่านั้น เพื่อไปยัง Goal และอย่าบอก Goal เพราะ goal คือหน้าสุดท้ายเราคลิกเองได้",
