-- SPACE v1.2 — AlphaGo WikiRace (Full Fix + Color Fix)
-- 🐷 Pink Pig Edition
...
local function getColorCategory(color)
    ...
    if diff < 25 then return nil end
    ...
    if hue >= 15 and hue <= 50 and r > 150 then return "orange" end
    if (hue <= 20 or hue >= 340) and r > 120 then return "red" end
    
    return nil
end

-- ==================== CIRCLE DETECTION (FIXED) ====================
...
-- ==================== CIRCLE COLOR (FIXED) ====================
local function getCircleColor(obj)
    if not obj then return nil end
    
    local color = nil
    
    if obj.BackgroundColor3 then
        color = obj.BackgroundColor3
    end
    
    if not color and obj:IsA("ImageLabel") and obj.ImageColor3 then
        color = obj.ImageColor3
    end
    ...
    -- 🆕 ลด threshold 25 → 15
    local isGray = math.abs(r - g) < 15 and math.abs(g - b) < 15 and math.abs(r - b) < 15
    if isGray then return nil end
    
    return {r = r, g = g, b = b}
end

-- ==================== STATUS COLOR (FIXED + DEBUG) ====================
local function getStatusColor()
    local DEBUG = false  -- 🆕 เปิดเป็น true ถ้าต้องการดู Log
    ...
    if not playerNameLbl then
        if DEBUG then print("[COLOR] ❌ หา PlayerName ไม่เจอ") end
        return "🔵"
    end
    ...
    if not playerCard then
        if DEBUG then print("[COLOR] ❌ หา PlayerCard ไม่เจอ") end
        return "🔵"
    end
    ...
    if DEBUG then 
        print("[COLOR] PlayerName = " .. playerName)
    ...
    for i = 1, math.min(3, #circles) do
        local color = getCircleColor(circles[i].obj)
        if color then
            activeCount = activeCount + 1
            if DEBUG then 
                print("[COLOR]   Circle " .. i .. ": RGB(" .. color.r .. "," .. color.g .. "," .. color.b .. ")")
            end
        else
            if DEBUG then print("[COLOR]   Circle " .. i .. ": gray") end
        end
    end
    
    if DEBUG then print("[COLOR] activeCount = " .. activeCount) end
    
    if activeCount == 1 then return "🔵"
    elseif activeCount == 2 then return "🟠"
    elseif activeCount >= 3 then return "🔴"
    else return "🔵" end
end
...
local function addLine(text, color)
    ...
    l.Parent = scroll
    local deferFn = task and task.defer or spawn
    pcall(function()
        deferFn(function()
            pcall(function()
                        ...
                        if not hasColor and obj.RichText then
                            ...
                            if not hasColor then
                                for r,g,b in text:gmatch('<font color="rgb%((%d+),(%d+),(%d+)%)"') do
                                    local cat = getColorCategory(Color3.fromRGB(tonumber(r),tonumber(g),tonumber(b)))
                                    if cat == "blue" or cat == "orange" or cat == "red" then
                                        hasColor = true; break
                                    end
                                end
...
local function findFuzzyMatch(pick, candidates)
    ...
    if #pickWords >= 1 then
        for _, c in ipairs(candidates) do
            ...
            if matchCount >= #pickWords then return c, "word-match" end
        end
    end
...
local function parsePicksFromText(text)
    ...
    if line1 and line2 then
        local answer = line2:match("^%s*คำตอบ%s*:%s*(.+)$")
                    or line2:match("^%s*Answer%s*:%s*(.+)$")
                    or line2:match("^%s*คำตอบ%s*(.+)$")
        if answer then
            answer = answer:gsub("^%s+", ""):gsub("%s+$", "")
            if #answer >= 2 and isEnglish(answer) then
                return {picks = {{rank = 1, title = answer}}, _raw = {plain = true}}
            end
        end
    end

    if not text:find("\n") and #text >= 2 and #text < 100 and isEnglish(text) then
        return {picks = {{rank = 1, title = text}}, _raw = {plain = true}}
    end
...
copyTargetBtn.MouseButton1Click:Connect(function()
    ...
    local statusColor = getStatusColor()
    local lines = {
        "CURRENT: " .. (currentArticle or "?") .. statusColor,
        "Target : " .. targetTitle,
    }
    local ok = copyToClipboard(table.concat(lines, "\n"))
    addLine("[🐽] " .. targetTitle .. " " .. statusColor .. (ok and " copied" or " FAIL"),
...
addLine("🐷 SPACE v1.2 — Color Fix", GREEN_OK)
addLine("小女孩 → Prompt | 777 → Fast", PINK_DEEP)
addLine("111 → Fallback | 🐽 → Target", ORANGE_WARN)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[SPACE v1.2 🐷] Loaded ✓")
