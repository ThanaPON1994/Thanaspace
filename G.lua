=====================================================================
-- 🎨 DRAW AI V15.0 — Harvard Quality Tier System
...
local drawQualityLevel = 8 -- ค่าเริ่มต้น LV8 (University Student)
...
function calculateQualityScore(strokes)
    ...
    -- Base score from strokes and points
    score = score + (strokeCount * 15)
    score = score + math.min(totalPoints * 0.5, 100)
    
    -- Balance score (aspect ratio closer to 1 = better)
    ...
    -- Coverage score (how much of bounding box is used)
    local coverage = 0
    if w > 0 and h > 0 then
...
-- ========== GET API OBJECTS (V15.0 — Level-based buffer & skip) ==========
function getAllJsonObjects(str, level)
    math.randomseed(tick() + os.time() + 9999)
    local config = getLevelConfig(level)
    local bufferSize = (config.buffer_mb or 3) * 1048576 -- MB to bytes
    ...
    -- อ่านข้อมูลตาม buffer size ของแต่ละ LV
    ...
    -- คำนวณ skip ตาม LV multiplier
    local effectiveSkip = math.floor(skipCount * (config.skip_multiplier or 2.5))
    effectiveSkip = math.max(1, math.min(effectiveSkip, 50)) -- cap ที่ 50
...
function isGoodDrawingByLevel(theme, strokes, level)
    ...
    -- 🎨 Level-based minimum requirements
    ...
    -- 🎨 Level-based quality thresholds
    ...
    -- 🔥 Theme-Specific Checks ( escalate with level )
    ...
    -- 🎨 Animal size checks (stricter at higher levels)
    if theme == "elephant" or theme == "giraffe" or theme == "lion" or theme == "horse" or theme == "dragon" or theme == "penguin" or theme == "flamingo" then
        ...
        local minSize = 80 + (level * 5) -- Higher level = bigger minimum
        if (animalWidth < minSize and animalHeight < minSize) then return false, qualityScore end
    end

    -- 🎨 Complex theme checks with level escalation
    local complexThemeChecks = {
        elephant = function() 
            if shapeLevel >= 3 then
                return hasExtension(strokes, "left") and hasExtension(strokes, "down")
            end
            return hasExtension(strokes, "left") 
        end,
        flamingo = function() 
            if shapeLevel >= 3 then
                return hasExtension(strokes, "down") and hasExtension(strokes, "right")
            end
            return hasExtension(strokes, "down") 
        end,
        broccoli = function() 
            local up = hasExtension(strokes, "up")
            local down = hasExtension(strokes, "down")
        ...
        broom = function() 
            if shapeLevel >= 3 then
                return hasExtension(strokes, "down") and totalPoints > 80
            end
            return hasExtension(strokes, "down") 
        end,
    }
...
-- ========== ASYNC API (V15.0 — Level-based filtering & quality scoring) ==========
function getGoogleQuickDrawAsync(theme, level, callback)
    ...
    -- 🎨 Clear cache if level changed for this theme
    ...
    task.spawn(function()
        ...
        if res and #res > 10 then
            ...
            -- 🎨 Select best quality drawing based on level
            if #candidates > 0 then
                table.sort(candidates, function(a, b) return a.score > b.score end)
                local selectedIndex = 1
                if level >= 13 then
                    selectedIndex = 1 -- Harvard: ต้องดีที่สุดเท่านั้น
                elseif level >= 10 then
                    selectedIndex = math.min(2, #candidates) -- Pro: top 2
                elseif level >= 7 then
                    selectedIndex = math.min(3, #candidates) -- Advanced: top 3
                elseif level >= 4 then
                    selectedIndex = math.min(5, #candidates) -- Beginner: top 5
                else
                    selectedIndex = math.min(8, #candidates) -- Noob: top 8
                end
        ...
        if finalStrokes and #finalStrokes > 0 then
            apiCache[cacheKey] = finalStrokes
        end
...
-- ========== DRAW THEME (V15.0 — Pass level to API) ==========
...
main.Size = UDim2.new(0, 320, 0, 540) -- เพิ่มความสูงสำหรับ LV Control
...
forceBtn.MouseButton1Click:Connect(function()
    if currentTheme then 
        ...
        -- 🎨 V15.0 — Clear ALL level caches for this theme
        for lv = 1, 15 do
            apiCache[normalized .. "_LV" .. lv] = nil
        end
...
-- 🎨 V15.0 — DRAW QUALITY LEVEL CONTROL (ใต้ Skip Count)
...
-- 🎨 V15.0 — LV Name Display (ขวาของตัวเลข)
...
local function updateLVDisplay()
    ...
    if drawQualityLevel <= 3 then
        lvCountLabel.TextColor3 = Color3.fromRGB(205, 127, 50) -- Bronze
        lvNameLabel.TextColor3 = Color3.fromRGB(205, 127, 50)
    elseif drawQualityLevel <= 6 then
        lvCountLabel.TextColor3 = Color3.fromRGB(192, 192, 192) -- Silver
        lvNameLabel.TextColor3 = Color3.fromRGB(192, 192, 192)
    elseif drawQualityLevel <= 9 then
        lvCountLabel.TextColor3 = Color3.fromRGB(255, 215, 0) -- Gold
        lvNameLabel.TextColor3 = Color3.fromRGB(255, 215, 0)
    elseif drawQualityLevel <= 12 then
        lvCountLabel.TextColor3 = Color3.fromRGB(185, 242, 255) -- Diamond
        lvNameLabel.TextColor3 = Color3.fromRGB(185, 242, 255)
    else
        lvCountLabel.TextColor3 = Color3.fromRGB(255, 0, 128) -- Crown/Harvard
        lvNameLabel.TextColor3 = Color3.fromRGB(255, 0, 128)
    end
end

lvDown.MouseButton1Click:Connect(function()
    if drawQualityLevel > 1 then
        drawQualityLevel = drawQualityLevel - 1
        updateLVDisplay()
        -- 🎨 ล้าง cache เมื่อเปลี่ยน LV
        getgenv().QuickDrawCache = {}
        apiCache = getgenv().QuickDrawCache
        log("🎨 LV Down → " .. drawQualityLevel .. " | Cache Cleared")
    end
end)

lvUp.MouseButton1Click:Connect(function()
    if drawQualityLevel < 15 then
        drawQualityLevel = drawQualityLevel + 1
        updateLVDisplay()
        -- 🎨 ล้าง cache เมื่อเปลี่ยน LV
        getgenv().QuickDrawCache = {}
        apiCache = getgenv().QuickDrawCache
        log("🎨 LV Up → " .. drawQualityLevel .. " | Cache Cleared")
    end
end)
...
-- UPDATE LOOP (V15.0 — Added LV stats)
task.spawn(function()
