...
-- 🆕 Fix: copy format ใหม่ + validation picks 1-60 + Goal leak warning + พับ GUI
-- 🆕 Fix: GUI ไม่ขึ้น — pcall ทุกจุดสำคัญ + WaitForChild timeout + Enum fallback
                ...
                if vis then
                    ...
                    if text ~= "" then
                        ...
                        if blue then
                            ...
                            if #disp > 0 and not isUiJunk(disp) then                                local key = disp:lower()
                                if not seen_texts[key] then
                                    seen_texts[key] = true
                                    local pos = obj.AbsolutePosition
                                    table.insert(items, {text=disp, x=pos and pos.X or 0, y=pos and pos.Y or 0, w=obj.AbsoluteSize and obj.AbsoluteSize.X or 0})
...
local function doGo()
    ...
    for _, p in ipairs(data.picks) do
        ...
        elseif candidateSet[t:lower()] then
            ...
            if not found then
                addLine("[reject] นอกลิสต์ 1-60: " .. t, RED_ERR)
            end
        end
    end
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    local lines = {
        "⚠️ เลือก 1 หน้า (1-60) เท่านั้น เพื่อไปยัง Goal และอย่าบอก/แนะนำ Goal",
        "CURRENT: " .. currentArticle,
        "Goal: " .. (targetTitle or "?"),
        "CANDIDATES:",
    }
    for i = 1, math.min(60, #filtered) do
...
addLine("🐷 v8.3 fixed", GREEN_OK)
addLine("小女孩 → Gemini → 777", PINK_DEEP)
addLine("validation 1-60 🐽", PINK_DARK)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v8.3 🐷 fixed] Loaded ✓")
