...
-- 🆕 กรอง UI junk เพิ่ม: The Free Encyclopedia / Choose the language / Article Read / General Knowledge
-- 🆕 LOG v2 Fallback (lastFallbackData)
-- 🆕 JSON Scanner (เลือก block ล่างสุด — รองรับ小女孩 output + JSON ด้านล่าง)
...
local lastFallbackData = nil   -- 🆕 LOG v2
...
local clgC = Instance.new("UICorner")
clgC.CornerRadius = UDim.new(0, 10)
clgC.Parent = copyLogBtn
...
local function navigateToTitle(title)
    ...
    if not targetId then
        ...
        -- 🆕 เก็บ fallback data สำหรับ LOG v2
        local fallbackList = {}
    ...
    lastFallbackData = nil   -- 🆕 เคลียร์ fallback เมื่อเจอ ID
    return true
end
...
local function parsePicksFromText(text)
    ...
    -- 🆕 สแกน { ทุกตัว → เก็บ JSON block ทั้งหมด
    local jsonBlocks = {}
    local pos = 1
    while true do
        ...
        if close then
            table.insert(jsonBlocks, text:sub(open, close))
            pos = close + 1
        else
            pos = open + 1
        end
    ...
    -- 🎯 ลองจาก "ล่างสุดขึ้นบน" — เลือก JSON block ที่ valid ล่างสุด
    for i = #jsonBlocks, 1, -1 do
        local c = jsonBlocks[i]
        local ok, data = pcall(function() return HttpService:JSONDecode(c) end)
        if ok and type(data) == "table" then
            -- 🆕 รองรับ key ภาษาไทย
            if data.picks or data.pick or data.title
                or data.คำตอบ or data.เลือก or data.รายการ
                or data.answer or data.choice then

                if data.คำตอบ and not data.picks then
                    local ans = tostring(data.คำตอบ)
                    if #ans >= 2 then
                        return {
                            picks = {{rank = 1, title = ans, _source = "thai-json"}},
                            _raw = {thai = true}
                        }
                    end
                end
...
local function toggleMin()
    guiVisible = false
    frame.Visible = false
    print("[ATLAS] ปิด GUI (RightShift เปิดกลับ)")
end
...
-- 🆕 Copy Log v2 — CURRENT / Target / Fallback
copyLogBtn.MouseButton1Click:Connect(function()
    ...
    if lastFallbackData and #(lastFallbackData.fallbacks or {}) > 0 then
        ...
        table.insert(lines, "🔍 ทางเลือกที่ใกล้เคียงที่สุดไปยัง " ..
            (lastFallbackData.target or "Target") .. " (Fallback Candidates):")
    ...
    if ok then
        addLine("[log-v2] copied " .. #payload .. " chars", GREEN_OK)
        statusLbl.Text = "📋 Log v2 copied"
    else
        addLine("[log-v2] copy FAIL", RED_ERR)
        statusLbl.Text = "Copy failed"
    end
end)

copyLogBtn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch then
        copyLogBtn.MouseButton1Click:Fire()
    end
end)
...
if articleUpdated then
    articleUpdated.OnClientEvent:Connect(function(payload)
        ...
        lastFallbackData = nil   -- 🆕 เคลียร์ fallback เมื่อเปลี่ยนเพจ
...
addLine("小女孩 → Gemini → 777", PINK_DEEP)
addLine("aggressive click 🐽", PINK_DARK)
addLine("LOG v2 fallback enabled", GREEN_OK)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v8.3 🐷 傳說中的龍女來了 Edition] Loaded ✓")
