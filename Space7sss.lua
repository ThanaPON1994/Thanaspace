-- SPACE v1.9 — AlphaGo WikiRace (Copy Target = Name only)
-- 🐷 Pink Pig Edition
...
local function getColorCategory(color)
    ...
    if hue >= 10 and hue <= 65 then
        if r > 100 and r > g and g >= b then return "orange" end
    end
...
copyTargetBtn.MouseButton1Click:Connect(function()
    ...
    addLine("[🐽] " .. targetTitle .. (ok and " copied" or " FAIL"),
...
addLine("🐷 SPACE v1.9 — Copy Target = Name", GREEN_OK)
addLine("小女孩 → Prompt | 777 → Fast", PINK_DEEP)
addLine("111 → Fallback | 🐽 → Target Name", ORANGE_WARN)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[SPACE v1.9 🐷] Loaded ✓")
