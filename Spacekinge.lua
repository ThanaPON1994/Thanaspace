-- ATLAS v9.3 — 🐽 EDITION ⚡
-- 🐷 Pink Pig Edition (350 x 304) — 傳說中的龍女來了
-- 🆕 小女孩 = SPEEDRUN prompt + CURRENT + TARGET + CANDIDATES
-- 🆕 ปุ่ม 🐽 = COPY TARGET ONLY (มุมขวาบน)
-- 🆕 ลบ GO — ใช้ 777 (paste + GO ทันที) แทน
-- 🆕 777 + CLEAR = เต็มแถว
-- 🆕 0 delay — ตัด task.wait ทุกจุดใน pasteBtn
-- 🆕 Fix: ปุ่ม - และ X มี debounce กัน trigger ซ้ำ
...
local TEAL        = Color3.fromRGB(60, 180, 180)
...
-- ==================== 📋 SPEEDRUN PROMPT ====================
local SPEEDRUN_PROMPT = [[# WikiRace SPEEDRUN v4
หลักการเดียว: ไปถึง TARGET ในจำนวนคลิกน้อยที่สุด

0. ก่อนเลือกทุกครั้ง ต้องทำ Candidate Scan ตามลำดับนี้:
   1) ถ้ามี TARGET อยู่ใน CURRENT หรือ candidates → เลือก TARGET ทันที จบ
   2) ถ้ามี candidate ที่เป็น anchor โดยตรงของ TARGET → เลือกก่อนทุกอย่าง
      - WORK: same creator, same universe, sequel/prequel, sibling series, studio
      - PERSON: ชาติ/รัฐ/เมืองเกิด, อาชีพ, ผลงานเด่น
      - PLACE: ประเทศ/รัฐ/เมือง/แหล่งน้ำ/ภูเขา
      - CONCEPT: สาขา/ทฤษฎี/บุคคลต้นคิด
      - ANIMAL: วงศ์/สกุล/ชนิด/ถิ่นเฉพาะ
   3) ถ้ามี candidate ที่เป็นผู้สร้าง/โปรดิวเซอร์/นักเขียนของ CURRENT และนำไป anchor ได้ → เลือกก่อน network/platform/genre
   4) ถ้าไม่มีจริง ๆ → ใช้ bridge ตามลำดับ

1. ระบุประเภทเป้าหมาย:
   PERSON / PLACE / THING / WORK / CONCEPT / ANIMAL / SPECIES

2. สร้าง Target Anchor สูงสุด 3 อัน:
   - PERSON → ชาติ / รัฐ / เมืองเกิด / อาชีพ / ผลงาน
   - PLACE → ประเทศ / รัฐ / เมือง / แหล่งน้ำ / ภูเขา
   - THING/WORK → ผู้สร้าง / สตูดิโอ / ซีรีส์แม่-ลูก / จักรวาลเดียวกัน / แพลตฟอร์ม
   - CONCEPT → สาขา / ทฤษฎี / บุคคลต้นคิด
   - ANIMAL → อนุกรมวิธาน + ถิ่นเฉพาะ

3. ลำดับสะพานที่ต้องใช้:
   1) TARGET โดยตรง
   2) anchor โดยตรงของ TARGET
   3) same creator / same universe / sibling series / studio
   4) ประเทศ → รัฐ → เมือง สำหรับ PERSON/PLACE
   5) อาชีพ / ผลงาน / รางวัล / สตูดิโอ / แพลตฟอร์ม เฉพาะที่จำเป็น
   6) อนุกรมวิธาน / ถิ่นเฉพาะ สำหรับ ANIMAL
   7) หมวดกว้าง ใช้เป็นทางสุดท้ายเท่านั้น

4. กฎห้าม:
   - ❌ ห้ามเลือกหมวดแม่ของ CURRENT
   - ❌ ห้ามใช้หมวดกว้างติดกันเกิน 1 คลิก
   - ❌ ห้ามวน hub ซ้ำ
   - ❌ ห้ามใช้ Humanities, Performing arts, Cinema, Television, Media, Internet,
        YouTube, Streaming media, Production company, Animation, Cartoon,
        Web series, Adult animation เป็นสะพานยาว
        เว้นแต่ TARGET หรือ anchor โดยตรงอยู่ในหมวดนั้น
   - ❌ ห้ามตอบว่า "ไม่ทราบ" "ไม่มีทาง" "ไม่สามารถตอบได้"
   - ❌ ห้ามใช้ General knowledge เป็นทางหลัก

5. กฎภูมิศาสตร์และ PERSON:
   - ถ้า TARGET เป็น PERSON → ใช้แกน ประเทศ → รัฐ/เมือง → บุคคล
   - ถ้า TARGET ไม่ใช่ภูมิศาสตร์ → ห้ามต่อเนื่องภูมิศาสตร์เกิน 1 คลิก
   - ถ้าเจอทวีป/ประเทศ/รัฐกว้าง → เปลี่ยนเป็นรัฐ/เมือง/ถิ่นเฉพาะทันที

6. ถ้าไม่มีทางชัดเจน:
   - ทำ Reverse BFS 2 hops จาก TARGET
   - เลือก candidate ที่อยู่บนเส้นทางนั้นก่อน
   - ถ้ายังไม่มี → ใช้สะพานเฉพาะทางที่ใกล้ที่สุด

7. ต้องคำนวณ clicks ทุกครั้ง

8. Output Format (JSON เท่านั้น):
{
  "target_type": "...",
  "target_anchor": ["...", "..."],
  "candidate_scan": {
  ...
  "path_strategy": "CURRENT → bridge → TARGET",
  "picks": [
    {"rank": 1, "title": "...", "why": "สะพานใกล้ที่สุด ไปถึง anchor ได้เร็วที่สุด"}
  ],
  "clicks": 0
}
]]
...
frame.Size = UDim2.new(0, 350, 0, 304)
...
-- ===== T: (สั้นลงเพื่อไม่ทับปุ่ม 🐽) =====
local infoLbl = Instance.new("TextLabel")
infoLbl.Size = UDim2.new(1, -120, 0, 20)
...
copyTargetBtn.Size = UDim2.new(0, 110, 0, 20)
copyTargetBtn.Position = UDim2.new(1, -116, 0, 4)
copyTargetBtn.BackgroundColor3 = TEAL
copyTargetBtn.Text = "🐽"
...
currentLbl.Size = UDim2.new(1, -12, 0, 20)
currentLbl.Position = UDim2.new(0, 6, 0, 28)
...
divider.Position = UDim2.new(0, 6, 0, 54)
divider.BackgroundColor3 = PINK_DEEP
...
hopBtn.Size = UDim2.new(1, -12, 0, 30)
hopBtn.Position = UDim2.new(0, 6, 0, 60)
...
statusLbl.Position = UDim2.new(0, 6, 0, 98)
...
pasteLbl.Size = UDim2.new(1, -12, 0, 12)
pasteLbl.Position = UDim2.new(0, 6, 0, 116)
...
pasteBtn.Size = UDim2.new(0.5, -9, 0, 26)
pasteBtn.Position = UDim2.new(0, 6, 0, 176)
...
clearJsonBtn.Size = UDim2.new(0.5, -9, 0, 26)
clearJsonBtn.Position = UDim2.new(0.5, 3, 0, 176)
...
scroll.Size = UDim2.new(1, -12, 1, -208)
scroll.Position = UDim2.new(0, 6, 0, 208)
...
local function getFilteredCandidates()
    ...
    if #filtered > MAX_CANDIDATES then
        local capped = {}
        for i = 1, MAX_CANDIDATES do capped[i] = filtered[i] end
        filtered = capped
    end
    return filtered
end
...
local function doGo()
    ...
    for i = 1, math.min(3, #data.picks) do
        ...
        if p.confidence then tag = tag .. " [" .. tostring(p.confidence):sub(1, 4) .. "]" end
...
-- 🆕 Debounce ปุ่ม minimize
local lastMinToggle = 0
local function toggleMin()
    local now = tick()
    if now - lastMinToggle < 0.3 then return end
    lastMinToggle = now
...
local function toggleClose()
    local now = tick()
    if now - lastCloseToggle < 0.3 then return end
    lastCloseToggle = now
...
-- ============ 小女孩 = COPY SPEEDRUN PROMPT ============
hopBtn.MouseButton1Click:Connect(function()
    if not currentArticle then refreshURL() end
    if not targetTitle or targetTitle == "" then
        addLine("[X] no target — รอ RoundStarted", RED_ERR)
        return
    end

    local cur = currentArticle or "?"
    local tgt = targetTitle

    local cands = {}
    local ok1, res = pcall(getFilteredCandidates)
    if ok1 and type(res) == "table" then cands = res end
    ...
    if #cands == 0 then
        table.insert(lines, "(ไม่มี candidates)")
    end

    local text = table.concat(lines, "\n")
    local ok = copyToClipboard(text)

    addLine("[📋] PROMPT + " .. #cands .. " cands " ..
        (ok and "copied ✓" or "FAIL"), ok and GREEN_OK or RED_ERR)
    addLine("     → paste ให้ Gemini", PINK_DARK)
    addLine("     → ตอบ JSON → paste → 777", PINK_DARK)
    statusLbl.Text = "📋 Prompt copied (" .. #cands .. " cands)"
    hopArticle = cur
end)

-- ============ 🐽 = COPY TARGET ONLY (มุมขวาบน) ============
copyTargetBtn.MouseButton1Click:Connect(function()
    if not targetTitle or targetTitle == "" then
        addLine("[X] no target", RED_ERR); return
    end
    local ok = copyToClipboard(targetTitle)
    addLine("[🐽] " .. targetTitle .. (ok and " ✓" or " FAIL"),
        ok and GREEN_OK or RED_ERR)
    statusLbl.Text = "🐽 Target copied"
end)

-- ============ 777 = paste + GO (0 delay!) ============
...
addLine("小女孩 = SPEEDRUN + cands", PINK_DEEP)
addLine("🐽 = copy target only", TEAL)
addLine("777 = paste + GO ⚡ (0 delay)", GREEN_OK)
statusLbl.Text = "🐽 Ready ⚡"
updateLabels()

print("[ATLAS v9.3 🐽 0-delay] Loaded ✓")
