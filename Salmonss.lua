-- ATLAS v9.6 — FORWARD-ONLY CHAIN + SMART AUTO v2 ⚡
-- 🐷 Pink Pig Edition (350 x 300) — 傳說中的龍女來了
-- 🆕 AUTO v2: Anti-Stuck + Hubness + Failed-Attempt Penalty
-- 🆕 Pending vs Visited: แยก "พยายาม" กับ "ไปสำเร็จ"
-- 🆕 RED v4 HUE-BASED: ตรวจจับเฉพาะวงกลมแดงแท้
...
local currentPath = {}
local visitedSet = {}    -- ✅ ยืนยันว่าไปแล้ว (ArticleUpdated ยืนยัน)
local pendingSet = {}    -- 🆕 พยายามแล้ว แต่ยังไม่ยืนยัน
local linkHubness = {}   -- 🆕 นับ link โผล่กี่หน้า = "hub"
local recentPicks = {}   -- 🆕 {title_lower = timestamp} — เพิ่งเลือกไป
local failedPicks = {}   -- 🆕 {title_lower = count} — คลิกพลาด
...
local function markVisited(name)
    ...
    pendingSet[name:lower()] = nil
    failedPicks[name:lower()] = nil
end
...
local function markPending(name)
    if not name or name == "" then return end
    pendingSet[name:lower()] = true
    recentPicks[name:lower()] = tick()
end

local function isRecentlyPicked(name, window)
    if not name or name == "" then return false end
    local t = recentPicks[name:lower()]
    if not t then return false end
    return (tick() - t) < (window or 10)
end

local function markFailed(name)
    if not name or name == "" then return end
    local k = name:lower()
    failedPicks[k] = (failedPicks[k] or 0) + 1
    pendingSet[k] = nil
    recentPicks[k] = tick()
end
...
local STOPWORDS = {
    ...
    ["new"]=true, -- ❗ ตัด "new" ออกเพราะ Platyrrhini New โผล่ซ้ำ
}
...
local function countSharedWords(a, b)
    local wa, wb = getWords(a), getWords(b)
    local n = 0
    for w in pairs(wa) do
        if wb[w] then n = n + 1 end
    end
    return n
end

-- 🆕 ขยายคำเป้า — person → อาชีพ/สัญชาติ/ยุค
local TARGET_EXPANSIONS = {
    ["hogan"] = {"wrestler","wrestling","actor","american","entertainer","wwf","wwe","terry","bollea"},
    ["einstein"] = {"physicist","physics","scientist","german","nobel","relativity"},
    ["napoleon"] = {"emperor","french","france","general","bonaparte"},
    ["mozart"] = {"composer","austrian","music","classical","opera"},
    ["tesla"] = {"inventor","serbian","engineer","physics","electricity","alternating"},
    ["newton"] = {"physicist","physics","mathematician","english","gravity","principle"},
    ["darwin"] = {"naturalist","biologist","english","evolution","species"},
    ["shakespeare"] = {"playwright","english","poet","drama","theatre","sonnet"},
    ["christ"] = {"christianity","religion","jesus","bible","church"},
    ["buddha"] = {"buddhism","religion","india","enlightenment","monk"},
    ["pole"] = {"antarctica","arctic","geography","latitude","ice","polar"},
    ["climate"] = {"weather","temperature","environment","atmosphere"},
    ["monkey"] = {"primate","ape","animal","mammal","species","biology"},
}

local function expandTargetWords(targetLow)
    local expanded = {}
    for kw, subs in pairs(TARGET_EXPANSIONS) do
        if targetLow:find(kw, 1, true) then
...
local function scoreCandidate(candidate, current, target)
    local score = 0
    local cLow = tostring(candidate):lower()
    local tLow = tostring(target or ""):lower()
    local curLow = tostring(current or ""):lower()
    local cSup = superNormalize(candidate)
    local tSup = superNormalize(target or "")

    -- 🎯 1) ตรงเป๊ะ
    if tSup ~= "" and cSup == tSup then
        return 100000
    end
    if #tSup >= 4 and cSup:find(tSup, 1, true) then
        return 50000
    end
    if #cSup >= 4 and tSup:find(cSup, 1, true) then
        return 30000
    end

    -- 🧠 2) คำร่วม target
    if tLow ~= "" then
        local shared = countSharedWords(cLow, tLow)
        score = score + shared * 500
    end

    -- 🆕 3) คำขยายเป้า (person → อาชีพ)
    if tLow ~= "" then
        local expanded = expandTargetWords(tLow)
        for w in pairs(expanded) do
            if cLow:find(w, 1, true) then
                score = score + 200
            end
        end
    end
    ...
    -- 🧠 4) คำร่วม current
    if curLow ~= "" then
        local sharedCur = countSharedWords(cLow, curLow)
        score = score + sharedCur * 30
    end

    -- 🆕 5) HUB — link ที่โผล่หลายหน้า = "page กลาง" น่าจะไปต่อได้
    local hubness = linkHubness[cLow] or 0
    score = score + hubness * 15

    -- 📏 6) ชื่อสั้นได้เปรียบ
    local len = #candidate
    if len <= 25 then score = score + 25
    elseif len <= 40 then score = score + 10
    elseif len > 80 then score = score - 60
    end

    -- 📚 7) คำที่บ่งชี้เนื้อหา
    if cLow:find("history") or cLow:find("war") or cLow:find("science")
        ...
        or cLow:find("american") or cLow:find("national") or cLow:find("state") then
        score = score + 15
    end

    -- 🚫 8) ปี / list of ติดลบ
    if cLow:match("^%d+$") or cLow:match("^%d+th century")
        or cLow:match("^list of") then
        score = score - 100
    end

    -- 🚫🚫 9) เลือกล่าสุด → หักหนัก
    local t = recentPicks[cLow]
    if t then
        local age = tick() - t
        if age < 5 then
            score = score - 5000        -- เพิ่งเลือกไปเมื่อกี้ → หักหนักสุด
        elseif age < 15 then
            score = score - 1000
        elseif age < 30 then
            score = score - 300
    ...
    -- 🚫🚫🚫 10) คลิกพลาดแล้ว → blacklist
    local fails = failedPicks[cLow] or 0
    if fails >= 1 then
        score = score - fails * 2000
    end
...
autoBtn.Size = UDim2.new(0, 114, 0, 28)
autoBtn.Position = UDim2.new(0, 118, 0, 64)
...
stopAutoBtn.Size = UDim2.new(0, 110, 0, 28)
stopAutoBtn.Position = UDim2.new(0, 234, 0, 64)
    ...
    for bold in text:gmatch("%*%*(.-)%*%*") do
        ...
        if #bold >= 2 and #bold <= 80 then
            ...
            local labels = {
                ...
                "เหตุผล","คำตอบ","เป้าหมาย","หมายเหตุ","คำเตือน","กลยุทธ์","ขั้นตอน","สรุป",
            }
            local isLabel = false
...
local function startInstantChain(picks)
    ...
    if #autoChainQueue == 0 then
        addLine("[CHAIN] ไม่มี rank ถัดไปแล้ว (อยู่สูงสุด)", ORANGE_WARN)
        statusLbl.Text = "🐽 DONE"
        return
    end
    ...
    addLine("[CHAIN] เริ่ม " .. #autoChainQueue .. " หน้า ⚡⚡⚡ (forward-only)", GREEN_OK)
    fireNextChain()
end
...
local function doAutoExploreStep()
    ...
    -- 🚫 กรอง visited + pending
    local pool = {}
    for _, c in ipairs(all) do
        local k = c:lower()
        if not isVisited(c) and not pendingSet[k] then
            table.insert(pool, c)
        end
    end
    -- ถ้าหมดจริงๆ → ใช้ all แต่ตัด failed ที่เคยพลาดเยอะออก
    if #pool == 0 then
        for _, c in ipairs(all) do
            local k = c:lower()
            if (failedPicks[k] or 0) < 2 then
                table.insert(pool, c)
            end
        end
    end
    -- ถ้ายังว่าง → ใช้ all ยกเว้น pending
    if #pool == 0 then
        for _, c in ipairs(all) do
            local k = c:lower()
            if not pendingSet[k] then
                table.insert(pool, c)
            end
        end
    end
    ...
    for _, c in ipairs(pool) do
        table.insert(scored, {
            title = c,
            score = scoreCandidate(c, currentArticle, targetTitle),
        })
    end
    ...
    if r < 0.85 then
        pick = scored[1].title
        addLine("[AUTO] 🎯 smart → " .. pick
            .. " (s" .. scored[1].score .. ")", GREEN_OK)
    else
        local topN = math.min(5, #scored)
        local rand = math.random(1, topN)
        pick = scored[rand].title
        addLine("[AUTO] 🎲 explore → " .. pick
            .. " (s" .. scored[rand].score .. ")", ORANGE_WARN)
    end
    ...
    if targetTitle then
        ...
        for _, c in ipairs(pool) do
            if superNormalize(c) == tSup then
                pick = c
                addLine("[AUTO] 🏆 TARGET เจอ! → " .. c, GREEN_OK)
                break
            end
        end
    end
...
local function startAutoExplore()
    if not autoExploreActive then return end
    addLine("[AUTO] 🚀 เริ่ม SMART v2 🔵", GREEN_OK)
...
local function doGo()
    ...
    elseif data._raw and data._raw.path then srcTag = " (path)" end
    ...
    if #data.picks > 10 then
        addLine("  ... (อีก " .. (#data.picks - 10) .. ")", PINK_DARK)
    end
...
autoBtn.MouseButton1Click:Connect(function()
    ...
    if autoExploreActive then
        addLine("[AUTO] ⚡ ON (v2 anti-stuck)", GREEN_OK)
        if currentArticle then
...
stopAutoBtn.MouseButton1Click:Connect(function()
    ...
    autoChainActive = false
    setAutoUI(false)
    addLine("[AUTO] ⛔ STOP by user", RED_ERR)
...
clearJsonBtn.MouseButton1Click:Connect(function()
    ...
    autoChainQueue = {}
    chainHistory = {}
    addLine("[clear] cleared + chain stopped", PINK_DEEP)
...
if articleUpdated then
    articleUpdated.OnClientEvent:Connect(function(payload)
        ...
        -- 🆕 ตรวจ pending: ถ้าหน้าใหม่ = pending ที่ตั้งไว้ → success!
        if lastPickAttemptTitle then
            local lastKey = lastPickAttemptTitle:lower()
            if t:lower() == lastKey or t:lower():find(lastKey, 1, true)
                or lastKey:find(t:lower(), 1, true) then
                markVisited(lastPickAttemptTitle)
            else
                -- ไม่ตรง → คลิกพลาด
                markFailed(lastPickAttemptTitle)
                addLine("[!] failed pick: " .. lastPickAttemptTitle, ORANGE_WARN)
            end
            lastPickAttemptTitle = nil
        end
            ...
            for _, v in pairs(art.Links) do
                if type(v) == "table" and v.Title then
                    ...
                    if isEnglish(n) and not isUiJunk(n) then
                        ...
                        -- 🆕 นับ hubness
                        local k = n:lower()
                        linkHubness[k] = (linkHubness[k] or 0) + 1
                    end
                end
            end
        end
...
if roundStarted then
    roundStarted.OnClientEvent:Connect(function(p)
        ...
        if autoExploreActive then
            addLine("[AUTO] 🎮 Round started — v2 วิ่งเลย!", GREEN_OK)
            task.defer(startAutoExplore)
        end
    end)
end
...
addLine("⚡ v9.6 AUTO v2 (anti-stuck)", GREEN_OK)
addLine("pending/visited แยก + hubness + failed", PINK_DEEP)
addLine("AUTO หยุดเองไม่เป็นแล้ว ✅", PINK_DARK)
statusLbl.Text = "🐽 Ready ⚡"
updateLabels()

print("[ATLAS v9.6 ⚡ AUTO v2] Loaded ✓")
