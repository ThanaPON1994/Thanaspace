-- ATLAS v9.3 — Copy Hop + Copy Target + Paste Gemini + Auto Navigate (FULL FIX)
-- 🐷 Pink Pig Edition — No-Reject + Fuzzy Match + Candidate Lock + No ID Log
-- 🆕 Fix: Goal→Target, ย้ายกฎลงล่าง, ปุ่ม COPY LOG แยก ขวาสุด, คงปุ่ม 777/CLEAR ตำแหน่งเดิม
...
-- 🆕 ปุ่ม COPY LOG (NO ID) — ขวาสุด แยกจาก小女孩
    ...
    for _, p in ipairs(data.picks) do
        ...
        elseif candidateSet[t:lower()] then
            ...
            if match then
                ...
                addLine("[REJECT] " .. t .. " → ไม่อยู่ใน CANDIDATES! ห้ามใช้", RED_ERR)
            end
        end
    end
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    -- 🆕 สร้างข้อความ Copy: CURRENT + Target + CANDIDATES + กฎ (ด้านล่างสุด)
    local lines = {
    ...
    -- 🆕 กฎและคำสั่ง อยู่ด้านล่างสุด (Recency Bias)
    table.insert(lines, "")
    table.insert(lines, "--------------------------------------------------")
    ...
    table.insert(lines, "4. ตอบเป็น JSON ตามโครงสร้างที่กำหนดไว้ใน System Prompt เท่านั้น")
    table.insert(lines, "--------------------------------------------------")
...
if articleUpdated then
    articleUpdated.OnClientEvent:Connect(function(payload)
        ...
        -- 🆕 เคลียร์ Log No ID เมื่อเปลี่ยนหน้า
...
addLine("🐷 v9.3 Candidate-Lock + No ID Log", GREEN_OK)
...
print("[ATLAS v9.3 🐷 Candidate-Lock + No ID Log] Loaded ✓")
