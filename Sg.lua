-- ATLAS v9.2 — AUTO PLAY + Groq API + Auto Key Rotation + Model Selector
-- 🐷 Pink Pig Edition
...
-- ==================== 🤖 MODELS ====================
local GROQ_MODELS = {
    {id = "llama-3.1-8b-instant",                      label = "1.8b-fast"},
    {id = "llama-3.3-70b-versatile",                   label = "3.3-70b"},
...
keyLbl.MouseButton1Click:Connect(function()
    ...
    currentKeyIndex = 1
    updateModelDisplay()
    addLine("[KEY] reset all keys 🐽", GREEN_OK)
end)
...
-- เรียก Groq ด้วย key + model ที่ระบุ
...
-- 🆕 Auto Key Rotation + Model Fallback
local function askGroq()
        ...
        if not deadKeys[kIdx] then
            ...
            for mOff = 0, #GROQ_MODELS - 1 do
                ...
                if answer then
                    ...
                    -- 401/403 → key ตาย → mark ไว้
                    if tostring(err):find("401") or tostring(err):find("403") then
                        deadKeys[kIdx] = true
                        addLine("[KEY] K" .. kIdx .. " ใช้ไม่ได้ → mark 💀", RED_ERR)
                        break  -- ออกไปลอง key ถัดไป
                    end
                    -- 429 → key นี้หมดชั่วคราว → ลอง model อื่นใน key เดิม
                    -- อื่นๆ → ลอง model ถัดไป
                end
                task.wait(0.15)
            end
        end
    ...
    -- ครบทุก key → reset deadKeys (เผื่อหมดเวลาแล้ว)
    deadKeys = {}
...
addLine("🐷 v9.2 ready", GREEN_OK)
addLine("小女孩 | 🤖 | ▶ AUTO", PINK_DEEP)
addLine("Auto Key Rotation: ON", BLUE_KEY)
statusLbl.Text = "🐽 Ready."
updateLabels()

print("[ATLAS v9.2 🐷 Groq Auto-Rotate] Loaded ✓")
