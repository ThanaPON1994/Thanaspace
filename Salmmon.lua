-- ATLAS v9.9 — SMART PATH PICK + COPY LOG ⚡
...
local clickNum = 0
local lastPicks = {}
local isProcessing, hopArticle = false, nil
...
local function formatPathLog()
    local lines = {}
    table.insert(lines, "=== ATLAS v9.9 PATH LOG ===")
    ...
    table.insert(lines, "")
    if #pathLog == 0 then
        table.insert(lines, "(ยังไม่มี hop — ยังไม่เริ่มเดิน)")
    else
    ...
    if okCand and cands and #cands > 0 then
        ...
        table.insert(lines, "  (ยังไม่มี — ยังไม่เริ่มเกม)")
    end
...
local function copyPathLog()
    ...
    if ok then
        addLine("[COPY] ✓ " .. #pathLog .. " hops → clipboard", GREEN_OK)
...
local function startInstantChain(picks)
    ...
    for _, p in ipairs(picks) do
        ...
        if t ~= "" then
            ...
            if r > chainCurrentRank then table.insert(autoChainQueue, t) else skipped = skipped + 1 end
        end
    end
...
local TARGET_HINTS = {
    ...
    ["christ"]  = {"christianity","religion","jesus","bible","church","god"},
    ...
    ["monkey"]  = {"primate","ape","animal","mammal","species","biology","forest"},
...
local function scoreCandidateV12(candidate, target)
    ...
    -- 🎯 Target hint keywords (occupation/type)
    if tLow ~= "" then
        for key, hints in pairs(TARGET_HINTS) do
            if tLow:find(key, 1, true) then
                for _, h in ipairs(hints) do
                    if cLow:find(h, 1, true) then score = score + 400 end
                end
            end
        end
    end
...
local function smartPick(pool, target)
    ...
    for _, c in ipairs(pool) do
        table.insert(scored, {title=c, score=scoreCandidateV12(c, target)})
    end
    table.sort(scored, function(a, b) return a.score > b.score end)

    -- 🎯 ถ้าเจอ target ตรง/near-exact → ยิงเลย
    ...
    if math.random() < 0.80 then
        ...
        -- weighted among all (base = 1 เพื่อไม่ให้ wander มาก)
...
local function startAutoExplore()
    ...
    addLine("[AUTO] 🚀 v1.2 SMART PICK", GREEN_OK)
...
addLine("⚡ v9.9 SMART PICK v1.2", GREEN_OK)
addLine("80% top-3 / 20% explore", PINK_DEEP)
addLine("📋 LOG — copy ครบ + popup", PINK_DARK)
statusLbl.Text = "🐽 Ready ⚡"
updateLabels()

print("[ATLAS v9.9 ⚡ SMART PICK v1.2] Loaded ✓")
