...
-- 🆕 จับสีฟ้าทุกโทน (เข้ม/อ่อน/เทา/น้ำเงิน/ม่วง)
-- 🆕 รองรับ Plain Text + Merge scanBlue + Fallback Click
-- 🆕 X ปิดได้ + RightShift toggle + ครบทุกปุ่ม
-- 🆕 RULES10 multi-line + G6-G10
...
local function isUiJunk(text)
    ...
    -- 3. ยาวเกิน 100 ตัวอักษร = UI text
    if #low > 100 then return true end
    if #low < 2 then return true end
    
    -- 4. Wikipedia UI Patterns (hardcoded inside)
    local wiki_junk = {
        "the free encyclopedia", "free encyclopedia",
        "wikipedia", "wikimedia",
        "main page", "contents",
        "current events", "random article",
        "about wikipedia", "contact us",
        "donate", "contribute", "help",
        "learn to edit", "community portal",
        "recent changes", "upload file",
        "special pages", "permanent link",
        "page information", "cite this page",
        "wikidata item", "download as pdf",
        "printable version", "what links here",
        "related changes", "tools",
        "article", "read", "edit",
        "view history", "talk", "watch",
        "create account", "log in", "log out",
        "search", "toggle the table of contents",
        "appearance", "personal tools",
        "namespaces", "views",
        ...
        "mobile view",
        "choose the language you want prioritized in translating the articles!",
        "choose the language", "translate the articles",
        "from wikipedia", "from the free encyclopedia",
        "article read", "article talk", "article edit",
        "read edit", "edit view history",
        "read edit view history", "article read edit",
        "read view history", "view edit history",
        "article view history", "talk read",
        "jump to content", "jump to navigation",
        "jump to search", "toggle sidebar",
        "not logged in", "main menu",
        "move to sidebar", "hide", "show",
        "general", "read more",
        "show more", "see more",
        "view source", "edit source",
        "get shortened url", "download qr code",
        "print export", "add links",
        "search wikipedia", "search wikipedia...",
        "how to play", "solo mode", "daily race",
    }
    for _, j in ipairs(wiki_junk) do
        if low == j then return true end
    end
    
    -- 5. Pattern matching (prefix)
    ...
    if low:find("^from ") and #low < 15 then return true end
    ...
    if low:match("^talk%s+%a+") then return true end
    ...
    if low:match("^hide%s+") then return true end
    if low:match("^show%s+") then return true end
    if low:match("^download%s+") then return true end
    if low:match("^print%s+") then return true end
    if low:match("^get%s+shortened") then return true end
    if low:match("^add%s+links") then return true end
    if low:match("^search%s+wiki") then return true end
    
    if low:find("^read edit") then return true end
    if low:find("^article read") then return true end
    if low:find("^article talk") then return true end
    if low:find("^read view") then return true end
    
    return false
end
-- ═══════════════════════════════════════════════════════════
-- จบการแก้ไข — ที่เหลือเหมือน v7 เดิม
-- ═══════════════════════════════════════════════════════════
        ...
        if not obj:IsDescendantOf(gui) then
            if obj:IsA("TextButton") or obj:IsA("TextLabel") then
                ...
                if text ~= "" and not isUiJunk(text) then
                    local isLink = isBlue(obj.TextColor3)
                    if not isLink and obj.RichText and text:find("<font") then
                        for hex in text:gmatch('<font color="#(%x%x%x%x%x%x)"') do
                            local r = tonumber(hex:sub(1,2),16)
                            local g = tonumber(hex:sub(3,4),16)
                            local b = tonumber(hex:sub(5,6),16)
                            if isBlue(Color3.fromRGB(r,g,b)) then isLink = true; break end
                        end
                    end

                    if isLink then
                        local disp = text:gsub("<[^>]+>",""):gsub("^%s+",""):gsub("%s+$","")
                        if superNormalize(disp) == targetSuper then
                            local target = obj
                            ...
                            addLine("[fallback-click] " .. disp, ORANGE_WARN)
                            pcall(function() 
                                if target.Activate then target:Activate() end 
                            end)
                            pcall(function()
...
local function navigateToTitle(title)
    ...
    if not targetId then
        local norm = normalize(currentArticle)
        local scanned = blueWordCache[norm] or {}
        local targetSuper = superNormalize(normTitle)
        for _, t in ipairs(scanned) do
            if superNormalize(t) == targetSuper then
                addLine("[scan-found] " .. t .. " (บนจอ) แต่ไม่มี ID", ORANGE_WARN)
                break
            end
        end
    end
    ...
    if not targetId then
        if tryClickVisibleLink(title) then
            addLine("[GO] " .. title .. " (fallback click)", GREEN_OK)
            markVisited(title)
            statusLbl.Text = "Clicked " .. title .. " (fallback)"
            return true
        end

        addLine("[X] no id: " .. title, RED_ERR)
        addLine("  ⚠️ ไม่ใช่ลิงก์ในหน้านี้", ORANGE_WARN)
...
local function parsePicksFromText(text)
    ...
    if fenced then table.insert(attempts, fenced) end
...
local function doGo()
    ...
    local srcTag = data._raw and data._raw.plain and " (plain)" or ""
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    local rulesBlock = table.concat({
        "RULES10:",
        "1. ห้ามวนหน้าเดิม",
        "2. ห้ามย้อนประเทศ/เมือง/รัฐ ที่เลือกแล้ว",
        "3. อยู่ในประเทศที่เลือก (Japan → Tokyo → Shibuya)",
        "4. ห้ามสลับแพลตฟอร์ม (YouTube ≠ Spotify)",
        "5. มี candidate ลิงก์ตรง Target → เลือกทันที",
        "6. ไม่มีตรง → เลือก 1-hop ที่ใกล้สุด",
        "7. ตรงที่สุด ไม่เลือก hub กว้างถ้าไม่จำเป็น",
        "8. ตัดหน้าที่ทำให้ออกนอกเส้นทาง",
        "9. เท่ากัน → เรียงตามอักษร",
        "10. ตอบ JSON สั้น",
        "G6. Zoom-in อย่างเดียว: ทวีป→ประเทศ→รัฐ→เมือง→สถานที่ ห้ามขึ้นชั้นกลับ",
        "G7. ค้นเส้นทางจริง 2–3 ขั้นก่อนตอบ ไม่ตอบจากความรู้ล้วน",
        "G8. Hub ใช้ได้ครั้งเดียว (United States, Association football) แล้วออกทันที ห้ามอยู่ hub สองตาติด",
        "G9. ทุกตาต้องลดระยะห่างจาก Target ไม่งั้นตัดทิ้ง",
        "G10. มองหา \"ของที่อยู่ใน Target\" ย้อนกลับจาก Target มาหาสะพาน",
    }, "\n")
...
addLine("ทุกสีฟ้า + UI filter + RULES10 multi 🐽", PINK_DARK)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v8.3 🐷 Full Fix + RULES10 multi] Loaded ✓")
