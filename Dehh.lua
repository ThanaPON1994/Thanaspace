-- SPACE v1.5.1 — AlphaGo WikiRace (Real-time + Orange All Shades)
-- 🐷 Pink Pig Edition
...
local function getColorCategory(color)
    ...
    -- 🟠 Orange: ครอบคลุมทุกเฉด (10-65°) + R > G >= B
    if hue >= 10 and hue <= 65 then
        if r > 100 and r > g and g >= b then
            return "orange"
        end
    end

    -- 🔴 Red: 0-20° หรือ 340-360°
...
local function isCircle(obj)
    ...
    if corner then
        ...
        if radius.Scale >= 0.3 then return true end
    end
...
local function getCircleColor(obj)
    ...
    local color = nil
    if obj.BackgroundColor3 then color = obj.BackgroundColor3 end
    if not color and obj:IsA("ImageLabel") and obj.ImageColor3 then color = obj.ImageColor3 end
    if not color then return nil end
    ...
    -- 🆕 เกณฑ์สีเทา (OR) — ครอบคลุมส้มอ่อน
    local isGray = (
        (math.abs(r - g) < 20 and math.abs(g - b) < 20 and math.abs(r - b) < 20)
        or (r > 240 and g > 240 and b > 240)
        or (r < 30 and g < 30 and b < 30)
    )
...
local function getStatusColor()
    ...
    if not playerNameLbl then return "🔵" end
    ...
    for i = 1, math.min(3, #circles) do
        ...
        if color then
            local cat = getColorCategory(Color3.fromRGB(color.r, color.g, color.b))
...
addLine("🐷 SPACE v1.5.1 — Real-time + Orange Shades", GREEN_OK)
addLine("小女孩 → Prompt | 777 → Fast", PINK_DEEP)
addLine("111 → Fallback | 🐽 → Target", ORANGE_WARN)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[SPACE v1.5.1 🐷] Loaded ✓")
