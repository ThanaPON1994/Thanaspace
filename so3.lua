...
-- 🆕 HOP copy แนบ G6 / G11 / G12 / G13 (ไว้บนสุด)
...
-- ==================== 🆕 HOP COPY (rules ไว้บนสุด) ====================
local HOP_RULES = {
    "=== RULES (follow strictly) ===",
    "",
    "G6 — GEOGRAPHIC ZOOM-IN:",
    "  Move from an entity to its geographic anchor (city/region/country) when possible. Do NOT jump straight to broad historical/categorical nodes unless the CURRENT is already geographic.",
    "",
    "G11 — ENTITY → LOCATION FIRST:",
    "  If CURRENT is an organization / ship / railway / shipping line (e.g. Hamburg America Line, Holland America Line), prefer the candidate that makes it \"become geographic\" first — e.g. Hamburg (city) or Germany (country).",
    "  DO NOT jump directly to a broad historical state (e.g. German Empire) until CURRENT has been resolved to a geographic node consistent with G6.",
    "",
    "G12 — MUST BE ABLE TO REACH TARGET IN 2-3 HOPS:",
    "  Only pick a candidate if, from that node, you can reasonably expect within the next 1-2 rounds to see a candidate that moves toward TARGET (e.g. Africa).",
    "  If picking it leads only to generic Europe / language / system / category nodes with no visible route to TARGET within 2-3 hops → REJECT it.",
    "",
    "G13 — ALLOWED BRIDGE RELATIONS ONLY:",
    "  Accept only these bridge types:",
    "    • located / based in",
    "    • operates / routes to",
    "    • historically controlled / colonies in",
    "    • region-of / connected-to",
    "  Loose/vague relations (\"related to\", \"category of\", \"has people named X\", \"event happened in\", generic historical context with no trace to TARGET) → FAIL G13, REJECT.",
    "",
    "OUTPUT FORMAT (JSON only, no prose):",
    '{"picks":[{"rank":1,"title":"...","why":"...","confidence":0.0}, ...]}',
    "Rank 1-3 only. 'why' must cite which bridge type is used and how it moves closer to TARGET.",
    "",
    "=== GAME DATA ===",
}
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    -- 🆕 RULES ขึ้นก่อนเลย
    for _, r in ipairs(HOP_RULES) do
        table.insert(lines, r)
    end

    -- แล้วค่อยตามด้วยข้อมูลเกม
    table.insert(lines, "CURRENT: " .. currentArticle)
    ...
    addLine("[HOP] " .. #filtered .. " cand + rules " .. (ok and "copied" or "FAIL"),
        ok and GREEN_OK or RED_ERR)
    ...
    statusLbl.Text = "Paste into Gemini (rules first)"
end)
...
addLine("HOP rules now on TOP", PINK_DARK)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v8.3 🐷 HOP rules-first] Loaded ✓")
