-- ATLAS v10.4 — AUTO v2.0 GOAL-DIRECTED ⚡
-- Semantic + BM25 + Trajectory Awareness + Frontier Memory + Gemini Boost
-- 🐷 Pink Pig Edition (350 x 340)
...
local CYAN_GEM    = Color3.fromRGB(90, 200, 235)
local TEAL_FR     = Color3.fromRGB(100, 210, 170)
...
local DIACRITIC_MAP = {
    ...
    ["\197\176"]="w",["\197\177"]="w",["\197\178"]="y",["\197\179"]="y",["\197\180"]="w",["\197\181"]="w",["\197\182"]="y",["\197\183"]="y",
...
local function tokenize(s)
    ...
    for w in tostring(s):lower():gmatch("%a+") do
        if #w >= 3 and not STOPWORDS[w] then table.insert(toks, w) end
    end
    return toks
end

local function hashTok(s)
    local h = 1000003
    for i = 1, #s do
        h = (h * 1000003 + s:byte(i) * 31) % 1000000007
    end
    return h
end
...
local EMB_DIM = 64
local embedCache = {}
local function embedVec(title)
    local key = tostring(title or ""):lower()
    local e = embedCache[key]
    if e then return e end
    local v = {}
    for i = 1, EMB_DIM do v[i] = 0 end
    local toks = tokenize(key)
    if #toks == 0 and key ~= "" then table.insert(toks, key) end
    for _, tok in ipairs(toks) do
        local h = hashTok(tok)
        local idx = (h % EMB_DIM) + 1
        local sign = (math.floor(h / EMB_DIM) % 2 == 0) and 1 or -1
        v[idx] = v[idx] + sign
    end
    if key ~= "" then
        local h = hashTok(key)
    ...
    local norm = 0
    for i = 1, EMB_DIM do norm = norm + v[i] * v[i] end
    norm = math.sqrt(norm)
    if norm > 0 then
        for i = 1, EMB_DIM do v[i] = v[i] / norm end
    end
    embedCache[key] = v
    return v
end
...
local function dot(a, b)
    local s = 0
    for i = 1, EMB_DIM do s = s + a[i] * b[i] end
    return s
end

local tokSetCache = {}
local function tokSetOf(title)
    local k = tostring(title or ""):lower()
    local s = tokSetCache[k]
    if s then return s end
    local set, n = {}, 0
    for _, w in ipairs(tokenize(k)) do
...
local function jaccard(a, b)
    if a.n == 0 or b.n == 0 then return 0 end
    local inter = 0
    for w in pairs(a.set) do
        if b.set[w] then inter = inter + 1 end
    end
    local union = a.n + b.n - inter
    if union <= 0 then return 0 end
    return inter / union
end

-- BM25: score แต่ละ candidate title เทียบ query = tokens ของ target
local function bm25Scores(pool, queryTokens)
    local out = {}
    local N = #pool
    if N == 0 or #queryTokens == 0 then
        for i = 1, N do out[i] = 0 end
        return out
    end
    local docToks, df, totalLen = {}, {}, 0
    for i, c in ipairs(pool) do
        ...
        for _, w in ipairs(t) do
            if not seen[w] then seen[w] = true; df[w] = (df[w] or 0) + 1 end
        end
    end
    local avgdl = math.max(1, totalLen / math.max(1, N))
    local k1, b = 1.5, 0.75
    for i, t in ipairs(docToks) do
        local tf = {}
        for _, w in ipairs(t) do tf[w] = (tf[w] or 0) + 1 end
        local s = 0
        for _, q in ipairs(queryTokens) do
            local f = tf[q] or 0
            if f > 0 then
                local dq = df[q] or 0
                local idf = math.log(1 + (N - dq + 0.5) / (dq + 0.5))
                s = s + idf * (f * (k1 + 1)) / (f + k1 * (1 - b + b * (#t / avgdl)))
            end
        end
        out[i] = s
    end
...
-- 🆕 PHASE 2/3 state
local pathHistory = {}          -- URL ทุก hop
local progressHistory = {}      -- sim(current,target) ต่อ hop
local stallStreak, lastSim = 0, 0
local wanderMode = false
local estMinHops = 4
local articleCache = {}         -- [title] = {hop,n,hash}
local frontierBest, frontierCount = {}, 0
local externalBoost = {}        -- [titleLower] = rank จาก Gemini
local unreachable = {}          -- คลิกไม่ได้ในหน้าปัจจุบัน
local metrics = {}
...
pasteBox.ClearTextOnFocus = false; pasteBox.BorderSizePixel = 0; pasteBox.Parent = content
...
-- 🆕 PHASE 3: ปุ่ม ASK GEMINI + STATS
local askBtn = Instance.new("TextButton")
askBtn.Size = UDim2.new(0, 166, 0, 26); askBtn.Position = UDim2.new(0, 6, 0, 192)
askBtn.BackgroundColor3 = CYAN_GEM; askBtn.Text = "🧠 ASK GEMINI"
askBtn.TextColor3 = WHITE; askBtn.Font = Enum.Font.SourceSansBold
askBtn.TextSize = 11; askBtn.BorderSizePixel = 0; askBtn.Parent = content
Instance.new("UICorner", askBtn).CornerRadius = UDim.new(0, 10)

local statsBtn = Instance.new("TextButton")
statsBtn.Size = UDim2.new(0, 160, 0, 26); statsBtn.Position = UDim2.new(0, 178, 0, 192)
statsBtn.BackgroundColor3 = TEAL_FR; statsBtn.Text = "📊 STATS"
statsBtn.TextColor3 = WHITE; statsBtn.Font = Enum.Font.SourceSansBold
statsBtn.TextSize = 11; statsBtn.BorderSizePixel = 0; statsBtn.Parent = content
Instance.new("UICorner", statsBtn).CornerRadius = UDim.new(0, 10)

local scroll = Instance.new("ScrollingFrame")
scroll.Size = UDim2.new(1, -12, 1, -228); scroll.Position = UDim2.new(0, 6, 0, 222)
...
local function addPathLog(page, src)
    ...
    table.insert(pathHistory, page)
    pickCount[page:lower()] = (pickCount[page:lower()] or 0) + 1
    metrics[src or "?"] = (metrics[src or "?"] or 0) + 1
end
...
local function showLogPopup(text)
    ...
    ttl.TextColor3 = WHITE; ttl.Font = Enum.Font.SourceSansBold; ttl.TextSize = 13
    ttl.TextXAlignment = Enum.TextXAlignment.Left; ttl.ZIndex = 101; ttl.Parent = logPopup
    ...
    box.TextWrapped = true; box.MultiLine = true; box.ClearTextOnFocus = false
    box.Text = text; box.BorderSizePixel = 0; box.ZIndex = 101; box.Parent = logPopup
...
local function formatPathLog()
    local lines = {}
    table.insert(lines, "=== ATLAS v10.4 PATH LOG ===")
    table.insert(lines, "Start : " .. (startTitle or "?"))
    table.insert(lines, "Target: " .. (targetTitle or "?"))
    table.insert(lines, "Now   : " .. (currentArticle or "?"))
    table.insert(lines, "Hops  : " .. #pathLog .. "  estMin: " .. estMinHops
        .. "  ratio: " .. string.format("%.2f", #pathLog / math.max(1, estMinHops)))
    table.insert(lines, "Sim   : " .. string.format("%.3f", lastSim or 0)
        .. "  stall: " .. stallStreak .. "  mode: " .. (wanderMode and "WANDER" or "NORMAL"))
    table.insert(lines, "")
    if #pathLog == 0 then
...
local function buildStats()
    local lines = {}
    table.insert(lines, "=== ATLAS v10.4 STATS ===")
    table.insert(lines, "Start : " .. (startTitle or "?"))
    table.insert(lines, "Target: " .. (targetTitle or "?"))
    table.insert(lines, "Now   : " .. (currentArticle or "?"))
    table.insert(lines, string.format("Hops  : %d  estMin: %d  ratio: %.2f", #pathLog, estMinHops, #pathLog / math.max(1, estMinHops)))
    table.insert(lines, string.format("Sim   : %.3f  stall: %d  mode: %s", lastSim or 0, stallStreak, wanderMode and "WANDER ⚠" or "NORMAL"))
    table.insert(lines, "-- pick kinds (round) --")
    for k, v in pairs(metrics) do
        table.insert(lines, string.format("  %-10s %d", k, v))
    end
    table.insert(lines, "Success total : " .. successTotal)
    table.insert(lines, "Frontier      : " .. frontierCount)
    table.insert(lines, "Cached pages  : " .. countKeys(articleCache))
    table.insert(lines, "Gemini boosts : " .. countKeys(externalBoost))
    return table.concat(lines, "\n")
end
...
-- 🛠 แก้: log hop เฉพาะเมื่อ navigate สำเร็จจริง (แก้ hop บวม)
...
local function fireNextChain()
    ...
    local ok = navigateToTitle(nextTitle)   -- 🛠 log ใน navigate เมื่อสำเร็จเท่านั้น
    if not ok then task.defer(fireNextChain) end
end
...
-- ============================================================
-- 🎯 SMART SCORING v2.0 (Semantic + BM25 + Trajectory)
-- ============================================================
local TOPIC_HINTS = {
    ...
    ["goat"] = {"mammal","animal","livestock","farm","bovid","ruminant","herd","milK","dairy","pasture","grazing"},
    ...
    ["rodman"] = {"basketball","nba","bulls","rebound","player","sport"},
    ["basketball"] = {"nba","sport","team","player","court","dunk"},
}

local CATEGORIES = {
    animal = {"animal","mammal","species","biology","zoology","wildlife","genus","family","taxon","vertebrate","livestock","bovid","ruminant"},
    ...
    sport = {"sport","football","soccer","team","player","championship","league","basketball","baseball"},
...
local TOPIC_CATEGORY = {
    ...
    ["goat"]="animal", ["sheep"]="animal", ["cow"]="animal", ["pig"]="animal",
    ...
    ["shakespeare"]="person", ["lincoln"]="person",
    ["rodman"]="person", ["jordan"]="person",
}
...
local BRIDGE_WORDS = {
    ...
    "livestock","herd","pasture","dairy","ruminant","bovid",
}
...
local function computeStartContext()
    ...
    startContext = {
        cat = findCategory(sLow),
        country = findCountry(sLow),
        words = getWords(sLow),
        lower = sLow,
    }
end
...
-- 🆕 PHASE 1: dead-end penalty (list/archive/history/index/timeline/outline/disambiguation)
local function deadEndPenalty(cLow)
    local p = 0
    if cLow:find("^list of") then p = p - 3000 end
    if cLow:find("disambiguation", 1, true) then p = p - 3000 end
    if cLow:find("archive", 1, true) then p = p - 1200 end
    if cLow:find("timeline", 1, true) then p = p - 800 end
    if cLow:find("outline", 1, true) then p = p - 800 end
    if cLow:find("history", 1, true) then p = p - 600 end
    if cLow:find("index", 1, true) then p = p - 600 end
    return p
end

-- 🆕 PHASE 1: loopPenalty dynamic -500*cnt^2
...
-- ==================== SCORE v2.0 ====================
local function scoreV20(candidate, ctx, bm)
    ...
    if cLow:match("^%d+$") then return -99999, flags end
    if cLow:match("^%d+th century") then return -99999, flags end
    if cLow:match("^list of") then flags.dead = true end
    if cLow:match("^%d%d%d%d") and #candidate <= 8 then return -3000, flags end

    local tSup = ctx.tSup
    if tSup ~= "" and cSup == tSup then return 1000000, {sub=true} end
    if #cSup >= 4 and #tSup >= 4 then
        if tSup:find(cSup, 1, true) then return 200000, {sub=true} end
        if cSup:find(tSup, 1, true) then return 150000, {sub=true} end
    end

    local score = 0

    -- 🧠 PHASE 1: semantic cosine + jaccard + bm25
    local cos = dot(embedVec(candidate), ctx.tEmb)
    score = score + cos * 2500
    score = score + jaccard(ctx.tTok, tokSetOf(candidate)) * 1500
    score = score + (bm or 0) * 400

    -- lexical เดิม (ลดน้ำหนักลงเพราะมี semantic แล้ว)
    if ctx.tWords then
        for w in pairs(ctx.tWords) do
            if cLow:find(w, 1, true) then score = score + 800 end
        end
    end
    if ctx.tLower ~= "" then
        for key, hints in pairs(TOPIC_HINTS) do
            if ctx.tLower:find(key, 1, true) then
    ...
    if ctx.tCat and matchesCategory(cLow, ctx.tCat) then score = score + 300 end
    if ctx.tCountry and cLow:find(ctx.tCountry, 1, true) then score = score + 400 end

    -- 🌉 bridge: ปกติ +800, ตอน WANDER +2000 (reconnect ผ่าน hub)
    if isBridge(cLow) then
        score = score + (ctx.wander and 2000 or 800)
        flags.bridge = true
    end

    if ctx.sContext then
        if ctx.sContext.cat and matchesCategory(cLow, ctx.sContext.cat) then score = score + 150 end
        for w in pairs(ctx.sContext.words) do
            if cLow:find(w, 1, true) then score = score + 100 end
        end
    end

    if mismatchesTarget(cLow, ctx.tCat) then score = score - 800 end

    -- 🆕 dead-end penalty (x2 ตอน wander)
    local dep = deadEndPenalty(cLow)
    if ctx.wander then dep = dep * 2 end
    score = score + dep

    -- 🆕 PHASE 3: Gemini external boost
    local rk = ctx.externalBoost[cLow]
    if rk then
        score = score + math.max(5000, 60000 - rk * 2000)
        flags.ext = true
    end

    -- 🆕 PHASE 3: frontier memory bonus
    local fs = ctx.frontierBest[cLow]
    if fs and fs > 500 then
        score = score + math.min(3000, fs * 0.15)
        flags.front = true
    end

    -- 🆕 PHASE 3: เคยเป็นหน้าปัจจุบันมาแล้ว (articleCache) → หลีกเลี่ยง
    if articleCache[normalize(candidate)] then score = score - 4000 end

    -- 🆕 PHASE 1: loop penalty dynamic
    score = score + loopPenalty(candidate)
...
-- 🆕 PHASE 3: frontier prune
local function frontierAdd(titleLow, s)
    if s <= 500 or s >= 100000 then return end
    local old = frontierBest[titleLow]
    if old and old >= s then return end
    if not old then frontierCount = frontierCount + 1 end
    frontierBest[titleLow] = s
    if frontierCount > 300 then
        local arr = {}
        for k, v in pairs(frontierBest) do table.insert(arr, {k=k, v=v}) end
        table.sort(arr, function(a,b) return a.v > b.v end)
        frontierBest = {}
        frontierCount = 0
        for i = 1, math.min(150, #arr) do
            frontierBest[arr[i].k] = arr[i].v
            frontierCount = frontierCount + 1
        end
    end
end

local function roulette(scored, n)
    local total, ws = 0, {}
    for i = 1, n do
        ws[i] = math.max(1, scored[i].score + 100)
        total = total + ws[i]
    end
    ...
    for i = 1, n do
        acc = acc + ws[i]
        if r <= acc then return scored[i] end
    end
    return scored[n]
end

-- ==================== SMART PICK v2.0 ====================
local function smartPick(pool, ctx)
    if #pool == 0 then return nil, "empty" end
    local bm = bm25Scores(pool, ctx.tTokList)
    local scored = {}
    for i, c in ipairs(pool) do
        local s, fl = scoreV20(c, ctx, bm[i] or 0)
        scored[i] = {title=c, score=s, flags=fl}
    end
    table.sort(scored, function(a, b) return a.score > b.score end)

    -- จำ candidate ดี ๆ ไว้ข้ามหน้า (frontier memory)
    for i = 1, math.min(10, #scored) do
        frontierAdd(scored[i].title:lower(), scored[i].score)
    end

    local top = scored[1]
    if top.score >= 100000 then return top.title, "SUBSTRING" end

    -- 🆕 PHASE 2: WANDER + pool แย่มาก → บังคับ RECONNECT ด้วย cosine สูงสุด
    if ctx.wander and top.score < 150 then
        local bestC, bestV = nil, -2
        for _, s in ipairs(scored) do
            local v = dot(embedVec(s.title), ctx.tEmb)
            if v > bestV then bestV = v; bestC = s end
        end
        if bestC then return bestC.title, "RECONNECT" end
    end

    -- 🆕 PHASE 2: adaptive explore (gap-based, bandit-like)
    local gap = (#scored >= 2) and (scored[1].score - scored[2].score) or 1e9
    local ep
    if gap > 6000 then ep = 0.05
    elseif gap > 2000 then ep = 0.10
    elseif gap > 600 then ep = 0.20
    else ep = 0.30 end
    if ctx.wander then ep = ep * 0.5 end

    local chosen, kind
    if math.random() < ep then
        chosen = roulette(scored, math.min(8, #scored))
        kind = "explore"
    else
        chosen = top
        kind = "top"
    end
    if chosen.flags.ext then kind = "GEMINI"
    elseif chosen.flags.front and chosen.score >= 2000 then kind = "FRONTIER"
    elseif chosen.flags.bridge and chosen.score >= 2000 then kind = "BRIDGE" end
    return chosen.title, kind
end
...
local function doAutoExploreStep()
    ...
    for _, c in ipairs(all) do
        if not isVisited(c) and not unreachable[c:lower()] then table.insert(pool, c) end
    end
    if #pool == 0 then
        for _, c in ipairs(all) do
            if not unreachable[c:lower()] then table.insert(pool, c) end
        end
    end
    if #pool == 0 then pool = all end

    -- 🆕 PHASE 2: trajectory awareness → wander detection
    local ratio = #pathLog / math.max(1, estMinHops)
    wanderMode = (stallStreak >= 3) or (ratio > 3)
    statusLbl.Text = string.format("🧭 h%d r%.1f sim%.2f%s", #pathLog, ratio, lastSim or 0, wanderMode and " ⚠WANDER" or "")

    local tLower = (targetTitle or ""):lower()
    local ctx = {
        tLower = tLower,
        tSup = superNormalize(targetTitle or ""),
        tWords = getWords(tLower),
        tCat = findCategory(tLower),
        tCountry = findCountry(tLower),
        sContext = startContext,
        tEmb = embedVec(targetTitle or ""),
        tTok = tokSetOf(targetTitle or ""),
        tTokList = tokenize(targetTitle or ""),
        wander = wanderMode,
        frontierBest = frontierBest,
        externalBoost = externalBoost,
    }
    if not startContext then computeStartContext() end

    local pick, kind = smartPick(pool, ctx)
    if not pick then autoExploreBusy = false; return end

    local icon, col
    if kind == "SUBSTRING" then icon, col = "💥", PURPLE_HIT
    elseif kind == "BRIDGE" then icon, col = "🌉", GOLD_HIT
    elseif kind == "GEMINI" then icon, col = "💠", CYAN_GEM
    elseif kind == "FRONTIER" then icon, col = "🧠", TEAL_FR
    elseif kind == "RECONNECT" then icon, col = "🔄", ORANGE_WARN
    elseif kind == "top" then icon, col = "🎯", PINK_DEEP
    else icon, col = "🎲", ORANGE_WARN end
    addLine("[AUTO] " .. icon .. " → " .. pick .. " [" .. kind .. "]", col)

    local ok = navigateToTitle(pick)
    autoExploreBusy = false
    if not ok then
        -- 🛠 แก้ hop บวม: หน้าไหนคลิกไม่ได้ในหน้านี้ → blacklist เฉพาะหน้าปัจจุบัน
        unreachable[pick:lower()] = true
        metrics["failnav"] = (metrics["failnav"] or 0) + 1
        task.defer(function()
            if autoExploreActive then task.wait(AUTO_DELAY); doAutoExploreStep() end
        end)
    end
...
local function startAutoExplore()
    ...
    addLine("[AUTO] 🚀 v2.0 GOAL-DIRECTED delay " .. AUTO_DELAY, GREEN_OK)
...
local function doGo()
    ...
    if not data or not data.picks or #data.picks == 0 then
        addLine("[X] parse failed", RED_ERR); statusLbl.Text = "Bad JSON"; return end
    lastPicks = data.picks
    -- 🆕 PHASE 3: เก็บเป็น external boost ให้ smartPick ด้วย
    for i, p in ipairs(data.picks) do
        if i <= 10 and p.title and p.title ~= "" then
            externalBoost[p.title:lower()] = i
        end
    end
    addLine("[ok] " .. #data.picks .. " picks (+boost)", GREEN_OK)
    for i = 1, math.min(10, #data.picks) do
        local p = data.picks[i]
        addLine("  #" .. i .. ": " .. (p.title or "?") .. (p.rank and " r"..p.rank or ""), PINK_DEEP)
    end
    if not autoExploreActive then
        startInstantChain(data.picks)
    else
        addLine("[boost] AUTO จะใช้คำแนะนำ Gemini ประกอบ scoring", CYAN_GEM)
    end
end

-- 🆕 PHASE 3: สร้าง prompt สำหรับ Gemini
local function buildGeminiPrompt()
    local cands = getFilteredCandidates()
    local parts, len = {}, 0
    for _, c in ipairs(cands) do
        if len + #c + 3 > 3500 then break end
        table.insert(parts, c)
        len = len + #c + 3
    end
    local lines = {}
    table.insert(lines, "Wikipedia racing game. Start: " .. (startTitle or "?") .. ". Target: " .. (targetTitle or "?") .. ". Current page: " .. (currentArticle or "?") .. ".")
    table.insert(lines, "Pick the 5 links that most directly lead toward the Target (think: which page would link to the Target?).")
    table.insert(lines, 'Reply ONLY with JSON: {"picks":[{"rank":1,"title":"..."},{"rank":2,"title":"..."}...]}')
    table.insert(lines, "Candidates: " .. table.concat(parts, " | "))
    return table.concat(lines, "\n")
end
...
local function toggleMin()
    if content.Visible then
        ...
        content.Visible = true; frame.Size = UDim2.new(0, 350, 0, 340); minBtn.Text = "-"
    end
end
...
autoBtn.MouseButton1Click:Connect(function()
    ...
    if autoExploreActive then
        addLine("[AUTO] ⚡ ON (v2.0 goal-directed)", GREEN_OK)
...
askBtn.MouseButton1Click:Connect(function()
    if not currentArticle then addLine("[X] no current page", RED_ERR); return end
    local prompt = buildGeminiPrompt()
    local ok = copyToClipboard(prompt)
    if ok then
        addLine("[🧠] prompt copied (" .. #prompt .. " chars)", CYAN_GEM)
        statusLbl.Text = "🧠 → Gemini, วางตอบกลับ, กด 777"
    else
        addLine("[🧠] copy FAIL", RED_ERR)
    end
end)

statsBtn.MouseButton1Click:Connect(function()
    showLogPopup(buildStats())
end)
...
clearJsonBtn.MouseButton1Click:Connect(function()
    ...
    externalBoost = {}
    addLine("[clear]", PINK_DARK); statusLbl.Text = "🐽 Cleared"
end)
...
-- ==================== HOOKS ====================
local function contentHash(links)
    local h = 7
    for _, l in ipairs(links) do
        h = (h * 31 + hashTok(l:lower())) % 1000000007
    end
    return h
end
...
if articleUpdated then
    articleUpdated.OnClientEvent:Connect(function(payload)
        ...
        -- 🆕 PHASE 3: article cache + reset unreachable ต่อหน้าใหม่
        articleCache[t] = {hop = #pathLog, n = #links, hash = contentHash(links)}
        unreachable = {}

        -- 🆕 PHASE 2: progress tracking / stall / success
        if targetTitle and targetTitle ~= "" then
            local sim = dot(embedVec(t), embedVec(targetTitle))
            table.insert(progressHistory, sim)
            if #progressHistory > 40 then table.remove(progressHistory, 1) end
            if #progressHistory >= 2 then
                local d = sim - progressHistory[#progressHistory - 1]
                if d <= 0.01 then stallStreak = stallStreak + 1 else stallStreak = 0 end
            end
            lastSim = sim
            if t:lower() == targetTitle:lower() then
                successTotal = successTotal + 1
                local eff = estMinHops / math.max(1, #pathLog)
                addLine("🏆 TARGET REACHED! hops=" .. #pathLog .. " eff=" .. string.format("%.2f", eff), GREEN_OK)
                autoExploreActive = false
                autoChainActive = false
                setAutoUI(false)
                statusLbl.Text = "🏆 " .. #pathLog .. " hops"
            end
        end
...
if roundStarted then
    roundStarted.OnClientEvent:Connect(function(p)
        ...
        -- 🆕 reset ต่อรอบ
        progressHistory, stallStreak, wanderMode, lastSim = {}, 0, false, 0
        unreachable, externalBoost, frontierBest, frontierCount = {}, {}, {}, 0
        articleCache = {}
        metrics = {}
        estMinHops = 4
        if startTitle and targetTitle then
            local s0 = dot(embedVec(startTitle), embedVec(targetTitle))
            if s0 >= 0.5 then estMinHops = 3
            elseif s0 >= 0.25 then estMinHops = 4
            elseif s0 >= 0.1 then estMinHops = 5
            else estMinHops = 6 end
        end
        clickNum = 0
        autoChainActive, autoChainQueue, chainHistory, chainCurrentRank = false, {}, {}, 0
        updateLabels()
        if autoExploreActive then
            addLine("[AUTO] 🎮 Round started! estMin=" .. estMinHops, GREEN_OK)
...
addLine("⚡ v10.4 AUTO v2.0 GOAL-DIRECTED", GREEN_OK)
addLine("semantic+bm25+trajectory+frontier", PINK_DARK)
addLine("🧠 ASK GEMINI = prompt → paste → 777", CYAN_GEM)
addLine("delay = " .. AUTO_DELAY .. " ทุกจุด", PINK_DEEP)
statusLbl.Text = "🐽 Ready "
updateLabels()

print("[ATLAS v10.4 ⚡ AUTO v2.0 GOAL-DIRECTED] Loaded ✓")
