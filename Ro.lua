...
-- 🔥 Fix: Duplicate GUI removal + Complete Script
-- =====================================================================
...
-- ✅ ลบ GUI เก่าทิ้งทันที ป้องกันซ้อน
if playerGui:FindFirstChild("DrawAI_V14") then
    playerGui.DrawAI_V14:Destroy()
end

local remo = ReplicatedStorage.rbtsx_include and ReplicatedStorage.rbtsx_include.node_modules
if remo then remo = remo["@rbtsx"] end
...
function getAllJsonObjects(str)
    ...
    local baseBuffer = 1048576
    local lvMultiplier = 1 + (currentLV - 1) * 0.167
    ...
    if #objects > skipCount then
        local startIndex = skipCount + 1
        local newObjects = {}
        for i = startIndex, #objects do
            table.insert(newObjects, objects[i])
        end
        objects = newObjects
...
function isGoodDrawing(theme, strokes)
    ...
    local minStrokes = math.max(2, math.floor(currentLV / 5))
    local minPoints = 15 + (currentLV * 4)
    local minStrokePoints = 3 + math.floor(currentLV / 3)
    if strokesCount < minStrokes then return false end
    if totalPoints < minPoints then return false end
    local w = maxX - minX
    local h = maxY - minY
    ...
    for _, s in ipairs(strokes) do
        if #s >= 2 and #s[1] > 0 then
            if #s[1] < minStrokePoints then return false end
        end
    end
    ...
    local requiredQuadrants = math.min(4, 1 + math.floor(currentLV / 6))
    if quadrantCoverage < requiredQuadrants then return false end
    return true
end
...
function validateStrokes(theme, strokes)
    ...
    local minStrokePoints = 2 + math.floor(currentLV / 5)
    local minTotalPoints = 15 + (currentLV * 3)
    local minValidStrokes = math.max(2, math.floor(currentLV / 6) + 1)
    for _, s in ipairs(strokes) do
    ...
    if validStrokeCount < minValidStrokes then return false end
    if totalPoints < minTotalPoints then return false end
    return true
end
...
function strokesToPoints(strokes, size, ox, oy)
    ...
    local interpolationSteps = math.max(1, math.floor(currentLV / 5))
    ...
    local function interpolatePoints(x1, y1, x2, y2, steps)
        ...
        for i = 0, steps do
            local t = i / steps
            table.insert(points, {x = x1 + (x2 - x1) * t, y = y1 + (y2 - y1) * t})
        end
        return points
    end
    local allStrokes = {}
    for _, stroke in ipairs(strokes) do
        ...
        if type(stroke) == "table" and #stroke > 0 then
            if stroke[1] and type(stroke[1]) == "table" and stroke[1].x then
                for _, p in ipairs(stroke) do
                    if p.x and p.y then table.insert(rawPoints, {x = p.x, y = p.y}) end
                end
            elseif #stroke >= 2 and type(stroke[1]) == "table" then
                for i = 1, #stroke[1] do
...
local lvLabel = Instance.new("TextLabel"); lvLabel.Size = UDim2.new(0.9, 0, 0, 14); lvLabel.Position = UDim2.new(0.05, 0, 0.27, 0); lvLabel.BackgroundColor3 = Color3.fromRGB(15, 5, 30); lvLabel.Text = "⭐ LV: 1/25"; lvLabel.TextColor3 = Color3.fromRGB(255, 215, 0); lvLabel.TextSize = 7; lvLabel.Font = Enum.Font.Code; lvLabel.Parent = main
...
local lvDown = Instance.new("TextButton"); lvDown.Size = UDim2.new(0.12, 0, 0, 28); lvDown.Position = UDim2.new(0.04, 0, 0.57, 0); lvDown.BackgroundColor3 = Color3.fromRGB(255, 100, 100); lvDown.Text = "−"; lvDown.TextColor3 = Color3.fromRGB(255, 255, 255); lvDown.TextSize = 16; lvDown.Font = Enum.Font.GothamBold; lvDown.Parent = main
local lvCountLabel = Instance.new("TextLabel"); lvCountLabel.Size = UDim2.new(0.14, 0, 0, 28); lvCountLabel.Position = UDim2.new(0.17, 0, 0.57, 0); lvCountLabel.BackgroundColor3 = Color3.fromRGB(20, 20, 20); lvCountLabel.Text = "1"; lvCountLabel.TextColor3 = Color3.fromRGB(255, 215, 0); lvCountLabel.TextSize = 10; lvCountLabel.Font = Enum.Font.Code; lvCountLabel.Parent = main
local lvUp = Instance.new("TextButton"); lvUp.Size = UDim2.new(0.12, 0, 0, 28); lvUp.Position = UDim2.new(0.32, 0, 0.57, 0); lvUp.BackgroundColor3 = Color3.fromRGB(100, 255, 100); lvUp.Text = "+"; lvUp.TextColor3 = Color3.fromRGB(0, 0, 0); lvUp.TextSize = 16; lvUp.Font = Enum.Font.GothamBold; lvUp.Parent = main
lvDown.MouseButton1Click:Connect(function() currentLV = math.max(1, currentLV - 1); lvCountLabel.Text = tostring(currentLV); lvLabel.Text = "⭐ LV: " .. currentLV .. "/25"; log("⭐ LV ลดเหลือ: " .. currentLV) end)
lvUp.MouseButton1Click:Connect(function() currentLV = math.min(25, currentLV + 1); lvCountLabel.Text = tostring(currentLV); lvLabel.Text = "⭐ LV: " .. currentLV .. "/25"; log("⭐ LV เพิ่มเป็น: " .. currentLV) end)
...
main.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then drag = true; ds = i.Position; fs = main.Position end end)
...
task.spawn(function()
    while task.wait(0.2) do
        pcall(function()
            ...
            if loadingCount > 0 then statusLabel.Text = "⏳ Loading " .. loadingCount .. " theme(s)... LV" .. currentLV; statusLabel.TextColor3 = Color3.fromRGB(255, 200, 0)
            elseif preloadComplete then statusLabel.Text = "✅ Ready LV" .. currentLV; statusLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
            else statusLabel.Text = "⏳ Ready | LV" .. currentLV .. " | Delay: " .. DRAW_DELAY .. "s"; statusLabel.TextColor3 = Color3.fromRGB(100, 255, 100) end
...
function preloadPopularThemes()
    ...
    local popular = {"cat","dog","house","tree","car","sun","fish","flower","apple","bird","boat","cake","chair","circle","cloud","cup","face","fire","heart","horse","leaf","moon","star","square","triangle","crown","sheep","drums","donut","axe","lobster","popsicle","steak","trumpet","grapes","paint can","book","leg","fork","bicycle","light bulb","hot dog","t-shirt","palm tree","cell phone","birthday cake"}
    task.spawn(function()
