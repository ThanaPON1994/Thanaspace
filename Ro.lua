-- ATLAS v8.7 — AUTO PLAY + AI + Smart IP
-- 🐷 Pink Pig Edition
-- 🆕 ▶ AUTO: กดครั้งเดียว → AI วนเองจนถึง target
-- 🆕 Manual IP + Auto-save
-- 🆕 Model 3b/7b | Ctrl+1 = AI, Ctrl+2 = AUTO
...
local function loadManualIP()
    if getgenv and getgenv().ATLAS_OLLAMA_IP then
        return getgenv().ATLAS_OLLAMA_IP
    end
    if readfile then
        local ok, data = pcall(readfile, "atlas_ollama_ip.txt")
...
local function saveManualIP(ip)
    manualIP = ip
    if getgenv then getgenv().ATLAS_OLLAMA_IP = ip end
    if writefile then
        pcall(writefile, "atlas_ollama_ip.txt", ip)
    end
end
...
local function playDoneSound()
    if not AUTO_CONFIG.soundOnDone then return end
    pcall(function()
...
local function getCurrentURL()
    ...
    for _, text in ipairs(c) do
        ...
        if match then
            match = match:gsub("%%(%x%x)", function(h) return string.char(tonumber(h, 16)) end)
...
local tc = Instance.new("UICorner"); tc.CornerRadius = UDim.new(0, 20); tc.Parent = titleBar
...
titleBar.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1
        ...
        if isClickOnButton and isClickOnButton(input.Position) then return end
...
local ic = Instance.new("UICorner"); ic.CornerRadius = UDim.new(0, 8); ic.Parent = infoLbl
...
statusLbl.Size = UDim2.new(1, -204, 0, 16)
...
ipInput.Size = UDim2.new(0, 105, 0, 16)
ipInput.Position = UDim2.new(0, 158, 0, 44)
ipInput.BackgroundColor3 = PINK_FIELD
ipInput.TextColor3 = PINK_DEEP
ipInput.PlaceholderText = "192.168.x.x"
ipInput.Text = ""
ipInput.Font = Enum.Font.Code
ipInput.TextSize = 10
ipInput.BorderSizePixel = 0
ipInput.ClearTextOnFocus = false
ipInput.Parent = content
local iic = Instance.new("UICorner"); iic.CornerRadius = UDim.new(0, 4); iic.Parent = ipInput

local setIPBtn = Instance.new("TextButton")
setIPBtn.Size = UDim2.new(0, 38, 0, 16)
setIPBtn.Position = UDim2.new(0, 266, 0, 44)
setIPBtn.BackgroundColor3 = GREEN_OK
setIPBtn.Text = "SET"
setIPBtn.TextColor3 = WHITE
setIPBtn.Font = Enum.Font.SourceSansBold
setIPBtn.TextSize = 10
setIPBtn.BorderSizePixel = 0
setIPBtn.Parent = content
local sс = Instance.new("UICorner"); sс.CornerRadius = UDim.new(0, 4); sс.Parent = setIPBtn

local rescanBtn = Instance.new("TextButton")
rescanBtn.Size = UDim2.new(0, 38, 0, 16)
rescanBtn.Position = UDim2.new(0, 307, 0, 44)
...
local rc = Instance.new("UICorner"); rc.CornerRadius = UDim.new(0, 4); rc.Parent = rescanBtn

-- Row y=64: 小女孩 | 3b | 7b | 🤖 | ▶ AUTO
...
for i, opt in ipairs(MODEL_OPTIONS) do
    ...
    b.Position = UDim2.new(0, 100 + (i-1) * 40, 0, 64)
...
aiBtn.Position = UDim2.new(0, 182, 0, 64)
...
autoBtn.Size = UDim2.new(0, 102, 0, 28)
autoBtn.Position = UDim2.new(0, 244, 0, 64)
...
local gc = Instance.new("UICorner"); gc.CornerRadius = UDim.new(0, 10); gc.Parent = goBtn
...
local function normalizePicks(data)
    ...
    local function pushTitle(entry, defaultRank)
        ...
        table.insert(out, {
            ...
            title = tostring(t),
            why = entry.why or entry.reason or entry.reasoning or entry.description,
            confidence = entry.confidence or entry.score,
        })
    end
...
local function parsePicksFromText(text)
    ...
    for bold in text:gmatch("%*%*(.-)%*%*") do
        ...
        if #bold >= 2 and #bold <= 80 then
            ...
            if not hasThai and isEnglish(cleanBold) then
                local found = false
...
- Format: {"picks":[{"rank":1,"title":"Exact Name","why":"reason"}]}
- "title" MUST be copied EXACTLY from CANDIDATES list.
]], currentArticle or "?", targetTitle or "?", table.concat(candLines, "\n"))
end
...
local function testOllamaIP(ip, httpFn)
    local ok, res = pcall(function()
        return httpFn({
            Url = "http://" .. ip .. ":" .. AI_CONFIG.port .. "/api/tags",
            Method = "GET",
...
local function autoDetectOllama(onLog)
    ...
    if onLog then onLog("[IP] scanning 12 common IPs...", nil) end
    ...
    if onLog then onLog("[IP] ❌ กรอก IP ที่ช่อง manual", false) end
    return nil
end

local function getOllamaURL(onLog)
    if not manualIP then manualIP = loadManualIP() end
    if manualIP then
        local httpFn = getHTTP()
        if httpFn and testOllamaIP(manualIP, httpFn) then
            if onLog then onLog("[IP] using: " .. manualIP, true) end
            return "http://" .. manualIP .. ":" .. AI_CONFIG.port .. "/api/generate"
        else
            manualIP = nil
        end
    end
    ...
    if AI_CONFIG.detectedIP then
        saveManualIP(AI_CONFIG.detectedIP)
        return "http://" .. AI_CONFIG.detectedIP .. ":" .. AI_CONFIG.port .. "/api/generate"
    end
    return nil
end

-- ==================== ASK AI (returns string) ====================
local function askAIOnce()
    if not currentArticle then refreshURL() end
    ...
    if not ollamaURL then return nil, "no URL" end
    ...
    if not parseOk or not respData or not respData.response then return nil, "bad resp" end

    addLine("[AI] ✓ " .. string.format("%.1f", dt) .. "s", GREEN_OK)
    return respData.response, nil
end

-- ==================== ASK + GO (manual button) ====================
local function askLocalAI()
    if not AI_CONFIG.enabled then return false end
    local answer, err = askAIOnce()
    if not answer then
    ...
    task.spawn(function()
        ...
        while isAutoRunning and loopCount < AUTO_CONFIG.maxLoops do
            ...
            if not currentArticle then
                addLine("[AUTO] ⏳ รอหน้าโหลด...", ORANGE_WARN)
                task.wait(2)
                if not currentArticle then
                    errCount = errCount + 1
                    if errCount >= AUTO_CONFIG.maxErrors then break end
                end
            end

            -- เช็คถึง target หรือยัง
            if currentArticle and normalize(currentArticle):lower() == normalize(targetTitle):lower() then
                addLine("🎉 ถึง TARGET แล้ว! (" .. loopCount .. " ครั้ง)", GREEN_OK)
            ...
            local answer, err = askAIOnce()
            if not answer then
                addLine("[AUTO] ❌ " .. tostring(err), RED_ERR)
                errCount = errCount + 1
                ...
                local ok = doGo()
                if not ok then
                    addLine("[AUTO] ⚠️ go fail", ORANGE_WARN)
                end
                task.wait(AUTO_CONFIG.waitPerPage)
            end
        end

        if loopCount >= AUTO_CONFIG.maxLoops then
            addLine("[AUTO] ⏰ ถึง max " .. AUTO_CONFIG.maxLoops .. " loops", ORANGE_WARN)
        end
...
setIPBtn.MouseButton1Click:Connect(function()
    local ip = (ipInput.Text or ""):gsub("%s+", "")
    if ip == "" then
        statusLbl.Text = "ใส่ IP ก่อน"
        return
    end
    saveManualIP(ip)
    AI_CONFIG.detectedIP = ip
    addLine("[IP] ✅ saved: " .. ip, GREEN_OK)
    statusLbl.Text = "IP: " .. ip
end)
...
rescanBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if url then
            ipInput.Text = tostring(AI_CONFIG.detectedIP)
        end
    end)
end)
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Enum.KeyCode.One and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
        if not isProcessing and not isAutoRunning then
            isProcessing = true
            task.spawn(function() pcall(askLocalAI); isProcessing = false end)
        end
    end
...
local saved = loadManualIP()
if saved then
    manualIP = saved
    ipInput.Text = saved
    addLine("[IP] loaded: " .. saved, GREEN_OK)
end

addLine("🐷 v8.7 ready", GREEN_OK)
addLine("小女孩 | 777 | 🤖 | ▶ AUTO", PINK_DEEP)
addLine("Ctrl+1 = AI, Ctrl+2 = AUTO", PINK_DARK)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v8.7 🐷 AUTO PLAY] Loaded ✓")
