-- ATLAS v10.0 — AUTO v1.3 SMART PATH + COPY LOG ⚡
...
local visitedSet = {}
local pathLog = {}
local pickCount = {}    -- 🆕 นับจำนวนครั้งที่แต่ละหน้าโดนเลือก (กัน loop)
local skipped = {}      -- 🆕 หน้าเคยเจอแต่ไม่เลือก → ให้โบนัสคราวหลัง
local clickNum = 0
local lastPicks = {}
...
local function showLogPopup(text)
    ...
    ttl.Text = "📋 PATH LOG — Ctrl+A เลือกทั้งหมด แล้ว Ctrl+C"
...
local function formatPathLog()
    local lines = {}
    table.insert(lines, "=== ATLAS v10.0 PATH LOG ===")
...
local function copyPathLog()
    ...
    if ok then
        ...
        addLine("[COPY] ✗ copy ไม่ได้ — เปิด popup", ORANGE_WARN)
...
local TARGET_HINTS = {
    ...
    ["rpg"] = {"role-playing","game","fantasy","dragon","quest"},
    ["shooter"] = {"fps","game","gun","military","war"},
    -- 👤 PEOPLE
    ...
    ["paris"] = {"france","french","capital","city","eiffel"},
    ["tokyo"] = {"japan","japanese","capital","city"},
...
-- 🆕 v1.3: keywords ที่สื่อว่า link ไม่มีเนื้อหา → ตัดทิ้ง
local JUNK_WORDS = {
    "tweet","modding","forums","forum","open source","modifications",
    "testers","beta","patch","update","developer","blog",
}
...
-- 🆕 v1.3: ตรวจว่า target เป็นเกมไหม
local GAME_HINTS = {
    "silent hill","resident evil","mario","zelda","minecraft","pokemon",
    "halo","grand theft auto","call of duty","warcraft","doom",
    "terraria","undertale","hollow knight","final fantasy","game","rpg",
    "video game","shooter","platformer",
}

local function isGameTarget(tLow)
    for _, g in ipairs(GAME_HINTS) do
        if tLow:find(g, 1, true) then return true end
    end
    return false
end

local GAME_WORDS = {
    "video game","playstation","nintendo","xbox","konami","capcom",
    "sega","game","gaming","console","arcade","pc game","microsoft",
    "sony","steam","valve","rockstar","activision","blizzard","ubisoft",
    "gameplay","player","multiplayer","single-player",
}

local function scoreCandidateV13(candidate, target)
    ...
    local tSup = superNormalize(target or "")

    -- 🚫 Hard filter ปี/list
    if cLow:match("^%d+$") then return -5000 end
    if cLow:match("^%d+th century") then return -5000 end
    if cLow:match("^list of") then return -2000 end
    if cLow:match("^%d%d%d%d") and #candidate <= 8 then return -3000 end

    -- 🚫 JUNK filter
    for _, j in ipairs(JUNK_WORDS) do
        if cLow == j or cLow:find("^" .. j .. "$") then return -800 end
    end

    -- 🎯 EXACT
    if tSup ~= "" and cSup == tSup then return 1000000 end
    if #tSup >= 4 and cSup:find(tSup, 1, true) then return 100000 end
    if #cSup >= 4 and tSup:find(cSup, 1, true) then return 50000 end

    local score = 0

    -- 🧠 Shared words
    if tLow ~= "" then
        local tWords = getTargetWords(tLow)
    ...
    -- 🆕 v1.3: ถ้า target เป็นเกม → link เกมได้โบนัส
    if isGameTarget(tLow) then
    ...
    -- 📚 Wikipedia hub words
    if cLow:find("history") or cLow:find("war") or cLow:find("science")
       or cLow:find("university") or cLow:find("country") or cLow:find("city")
       or cLow:find("language") or cLow:find("culture") or cLow:find("people")
       or cLow:find("american") or cLow:find("national") or cLow:find("world") then
        score = score + 25
    end

    -- 🆕 v1.3: skip memory — เคยเจอแล้วไม่เลือก → ให้โอกาส
    if skipped[cLow] then
        score = score + 200 * math.min(skipped[cLow], 3)
    end
...
-- 🆕 v1.3: ตรวจ loop
local function isLoop(c)
    local cnt = pickCount[c:lower()] or 0
    return cnt >= 2
end

local function smartPick(pool, target)
    if #pool == 0 then return nil, "empty" end
    local scored = {}
    for _, c in ipairs(pool) do
        local s = scoreCandidateV13(c, target)
        -- 🆕 v1.3: penalize loop
        if isLoop(c) then
            s = s - 3000
        end
    ...
    -- 🎯 ยิง target เลย
    if scored[1].score >= 50000 then
        return scored[1].title, "target"
    end
...
local function doAutoExploreStep()
    ...
    -- 🆕 v1.3: กรอง visited + loop
    local pool = {}
    for _, c in ipairs(all) do
        if not isVisited(c) and not isLoop(c) then
            table.insert(pool, c)
        end
    end
    -- ถ้าติด loop หมด → ผ่อนปรน
    if #pool == 0 then
        for _, c in ipairs(all) do
    ...
    -- 🆕 เก็บ "skipped" ที่ถูกกรอง → ถ้าโผล่ใหม่ ให้ priority
    for _, c in ipairs(all) do
        local k = c:lower()
        if not isVisited(c) and not isLoop(c) then
            skipped[k] = (skipped[k] or 0) + 1
        end
    end

    local pick, kind = smartPick(pool, targetTitle)
    if not pick then autoExploreBusy = false; return end

    addPathLog(pick, kind)
    addLine("[AUTO] " .. (kind == "target" and "🏆" or kind == "top" and "🎯" or "🎲") .. " → " .. pick, kind == "target" and GREEN_OK or PINK_DEEP)
...
local function startAutoExplore()
    ...
    addLine("[AUTO] 🚀 v1.3 SMART", GREEN_OK)
...
addLine("⚡ v10.0 AUTO v1.3", GREEN_OK)
addLine("Loop prev + Game detect + Skip memory", PINK_DEEP)
addLine("85% top-3 / 15% explore", PINK_DARK)
statusLbl.Text = "🐽 Ready ⚡"
updateLabels()

print("[ATLAS v10.0 ⚡ AUTO v1.3] Loaded ✓")
