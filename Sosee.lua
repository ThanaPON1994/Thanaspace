...
-- 🆕 RULES10 + G6-G15 แปะบนสุดทุกครั้งที่ COPY HOP
-- 🆕 เปลี่ยนชื่อปุ่ม: COPY HOP → 777, PASTE + GO → space
...
-- ==================== 🆕 RULES HEADER (สำหรับ COPY HOP) ====================
local RULES_HEADER = [[# ภารกิจ
ให้เลือก **1 ตัวเท่านั้น** จาก `CANDIDATES` เพื่อพา `CURRENT` ไปยัง `TARGET` โดยต้องทำตามกฎทั้งหมดด้านล่าง และตอบเป็น JSON สั้นแบบนี้เท่านั้น:
`{"pick":{"title":"..."}}`

# ห้ามทำ
- ห้ามถามคำถามเพิ่ม
- ห้ามบอกว่า "ไม่มั่นใจ/ยังไม่แน่ใจ/ต้องตรวจเพิ่ม"
- ห้ามอธิบายยาว
- ห้ามเลือกมากกว่า 1 ตัว

# RULES10
1. ห้ามวนหน้าเดิม
2. ห้ามย้อนประเทศ/เมือง/รัฐ/ระดับที่เลือกแล้ว
3. อยู่ในประเทศที่เลือก (ต้อง zoom-in เท่านั้น) (ตัวอย่าง: Japan -> Tokyo -> Shibuya)
4. ห้ามสลับแพลตฟอร์ม
5. ถ้ามี candidate ลิงก์ตรง Target -> เลือกทันที
6. ถ้าไม่มีตรง -> เลือก 1-hop ที่ใกล้สุด
7. ตรงที่สุด ไม่เลือก hub กว้างถ้าไม่จำเป็น
8. ตัดหน้าที่ทำให้ออกนอกเส้นทาง
9. ถ้าเท่ากัน -> เรียงตามอักษร
10. ตอบเป็น JSON สั้นเสมอ

# G6 (Zoom-in อย่างเดียว)
- ต้องทำ **ทวีป -> ประเทศ -> รัฐ/จังหวัด -> เมือง -> สถานที่**
- ห้ามขึ้นชั้นกลับ

# G7 (ต้องยืนยันเส้นทางจริง 2-3 ขั้น)
- ต้องยืนยันว่า "มีทางไปต่อ" ได้ 2-3 ขั้นจริงจาก candidate ที่เลือก
- ห้ามตอบจากความรู้ล้วน

# G8 (Hub ใช้ได้ครั้งเดียว)
- Hub ใช้ได้ครั้งเดียว แล้วต้องออกทันที
- ห้ามอยู่ hub สองตาติด

# G9 (ลดระยะห่าง)
- ทุกการเลือกต้อง "ลดระยะห่างจาก Target"
- ถ้าไม่ลด -> ตัดทิ้ง

# G10 (มองย้อนจาก Target หาสะพาน)
- มองย้อนจาก `TARGET` เพื่อหา "ของที่อยู่ใน Target"
- เลือก candidate ที่เป็นสะพานเชื่อมกลับมายัง `CURRENT` อย่างมีเหตุผล

# G11 (กันหลุด/กันลูปด้วยการไล่ขั้น)
- ห้ามเลือกสิ่งที่ทำให้ดูเหมือนถูกทาง แต่พาให้กลับไปอยู่ "ไกลกว่าเดิม"
- ถ้ามีทางที่คาดว่าจะไปถึง "ใกล้ขึ้น" ได้ใน 1-2 step ให้เลือกทางนั้นก่อน

# G12 (เงื่อนไข "เข้าใกล้แบบส้ม" แบบตัดสินเชิงปฏิบัติ)
- เลือกได้ก็ต่อเมื่อในรอบถัดไป (1-hop) ยังมี candidate ในชุดที่ "เป็นแนวทางไป Target ชัด"
- ถ้าเลือกแล้วเหลือแต่เรื่องทั่วไป/ไม่เห็นทางไป Target ใน 1-2 รอบ -> ตัดทิ้งทันที

# G13 (กันกระโดดชนิดโหนดผิดบริบท)
- ถ้า `CURRENT` เป็น **องค์กร/เรือ/บริษัทรถไฟ/สายการเดินเรือ**
  ให้ zoom-in ไปสู่ **ที่ตั้งจริง** ก่อน (เมือง/ประเทศ/ภูมิภาค)
- ห้ามกระโดดไปเป็น **รัฐ/โครงสร้างประวัติศาสตร์กว้าง** ก่อนที่ CURRENT จะกลายเป็น "ภูมิศาสตร์" ตาม G6

# G14 (Priority Lock แบบเด็ดขาด)
ให้เลือกตามลำดับนี้โดย "หยุดทันทีเมื่อหาเจอ":
1) ถ้ามี candidate ที่ "เชื่อมตรง Target" -> เลือกตัวนั้น
2) มิฉะนั้น เลือกตัวที่ทำให้ "ไม่เพิ่มระยะ" และผ่าน G6 + G13
3) จากตัวที่เหลือ เลือกตัวที่ทำให้ "คาดได้ว่าใน 1-2 รอบจะไปแตะ Target ได้" (อิง G7 + G12)
4) ถ้ายังเหลือหลายตัว -> เลือก "ตรงที่สุด" ตาม G9/G10 แล้วเทียบตัวอักษร (G9 ข้อ 9)

# G15 (คำสั่งห้ามความลังเล)
- ต้องเลือก 1 ตัวแม้ชุดข้อมูลจะทำให้มั่นใจไม่ 100%
- แต่ต้องเลือกตาม Priority Lock (G14) เสมอ
- ห้ามใช้ถ้อยคำที่สื่อว่า "ไม่แน่ใจ/ต้องตรวจ/รอข้อมูล"

# รูปแบบคำตอบ (ห้ามเปลี่ยน)
ตอบ: `{"pick":{"title":"..."}}` เท่านั้น
...
-- 🆕 เปลี่ยนชื่อปุ่ม PASTE + GO → space
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    for i, c in ipairs(filtered) do
        table.insert(bodyLines, i .. ". " .. c)
    end

    local finalText = RULES_HEADER .. "\n" .. table.concat(bodyLines, "\n")
    local ok = copyToClipboard(finalText)

    addLine("[777-src] cache=" .. cacheCount .. " scan=" .. scanCount, PINK_DARK)
    addLine("[777] " .. #filtered .. " cand + RULES " .. (ok and "copied" or "FAIL"),
        ok and GREEN_OK or RED_ERR)
    ...
    statusLbl.Text = "Paste into Gemini (w/ RULES)"
end)
...
pasteBtn.MouseButton1Click:Connect(function()
    ...
    if txt and #txt > 0 then
        pasteBox.Text = txt
        addLine("[space] " .. #txt .. " chars", PINK_DARK)
        statusLbl.Text = "Pasted → GO"

        task.wait(0.1)
        doGo()
    else
        addLine("[space] empty", PINK_DEEP)
...
addLine("777 → Gemini → space", PINK_DEEP)
addLine("RULES10+G6-G15 แปะบนสุดทุกครั้ง 🐽", PINK_DARK)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v8.3 🐷 777+space Edition] Loaded ✓")
