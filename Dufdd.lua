-- ATLAS v9.4.4 — Copy Hop + Copy Target + Paste Gemini + Auto Navigate (FULL FIX)
-- 🐷 Pink Pig Edition — Cap hidden 250 + Cache 0.1 + pick_index + confirmation + No ID Log
-- 🆕 Fix: ลบ Loop 3 ตัวเลือก, ลบ FireServer อัตโนมัติ, เพิ่ม Cache scanBlue
...
-- 🆕 Cache สำหรับ scanBlue (v9.4.4)
local lastScanTime = 0
local lastScanResult = nil
local lastScanPage = nil
local SCAN_COOLDOWN = 0.1  -- 🆕 Cache 0.1 วิ
...
-- 🆕 ปุ่ม小女孩 (เต็มความกว้าง ไม่มีช่อง Cap)
...
local function scanBlue()
    -- 🆕 ใช้ Cache ถ้าสแกนภายใน 0.1 วิ และหน้าเดียวกัน
    local now = tick()
    if lastScanResult and lastScanPage == currentArticle and (now - lastScanTime) < SCAN_COOLDOWN then
        addLine("[scan] ใช้ cache (" .. math.floor((now - lastScanTime) * 1000) .. "ms)", PINK_DARK)
    ...
    lastScanPage = currentArticle
    addLine("[scan] สแกนใหม่ (" .. #result .. " คำ)", GREEN_OK)
...
-- ==================== CLICK ====================
-- 🆕 ปิดการคลิกอัตโนมัติ — ไม่เปลี่ยนหน้าเอง
...
local function navigateToTitle(title)
    ...
    if not targetId then
        addLine("[no-id] " .. title .. " → ไม่มี ID ไม่คลิกเอง", ORANGE_WARN)
...
local function doGo()
    ...
    if currentArticle then
        local norm = normalize(currentArticle)
        -- 🆕 ไม่ล้าง Cache (ให้ scanBlue ใช้ Cache)
    ...
    -- 🆕 ลองแค่ pick แรกเท่านั้น ไม่วน Loop
    local p = data.picks[1]
    local t = p.title or ""
    local tag = ""
    ...
    addLine("  #1: " .. t .. idxTag .. tag, PINK_DEEP)
    if p.why then
    ...
    -- 🆕 navigate แค่ครั้งเดียว
    local navigated = navigateToTitle(t)

    if navigated then
        statusLbl.Text = "Clicked 🐽"
    else
        statusLbl.Text = "No ID — กด COPY LOG"
        addLine("━━━━━━━━━━━━━━━━━━━━", GOLD)
        addLine("❌ หา ID ไม่เจอ", RED_ERR)
        addLine("👉 กด COPY LOG (ขวา) → ถาม AI ใหม่", GOLD)
        addLine("👉 แล้วกลับมากด 777", GOLD)
        addLine("━━━━━━━━━━━━━━━━━━━━", GOLD)
    end
end
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    -- 🆕 ไม่เช็ค Goal — แค่ Copy
    local norm = normalize(currentArticle)
    local cacheCount = #(gameCache[norm] or {})

    -- 🆕 สแกน (จะใช้ Cache ทันทีถ้ามี)
...
if articleUpdated then
    articleUpdated.OnClientEvent:Connect(function(payload)
        ...
        -- 🆕 เคลียร์ Cache scanBlue เมื่อเปลี่ยนหน้า
        if lastScanPage and lastScanPage ~= t then
            lastScanResult = nil
            lastScanPage = nil
            lastScanTime = 0
        end
