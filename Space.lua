-- ATLAS v8.9 — AUTO PLAY + Gemini API (Hardcoded Key + Auto-Model-Retry)
-- 🐷 Pink Pig Edition
-- 🆕 API Key ฝังใน script (ไม่ต้องกรอก GUI)
-- 🆕 รองรับ AQ. key ผ่าน x-goog-api-key header
-- 🆕 ลองโมเดลอัตโนมัติถ้าตัวแรก error (401/404/429)
-- 🆕 AUTO loop + Manual mode
...
-- ==================== 🔑 HARDCODED KEY ====================
local HARDCODED_KEY = "AQ.Ab8RN6JccWiBOS9tIuGixMe1A1dRmoZdD0BBfoGksWMrp3icpA"

-- ==================== 🤖 MODELS (ลองเรียงตามลำดับ) ====================
local GEMINI_MODELS = {
    "gemini-2.5-flash",
...
statusLbl.Text = "🐽 Ready. [KEY loaded]"
...
pasteBtn.Position = UDim2.new(0, 6, 0, 174)
...
clearJsonBtn.Position = UDim2.new(0, 120, 0, 174)
...
goBtn.Position = UDim2.new(0, 234, 0, 174)
...
scroll.Size = UDim2.new(1, -12, 1, -210)
...
-- เรียก Gemini 1 ครั้งด้วย model ที่กำหนด
local function callGemini(model, prompt)
    local http = getHTTP()
    ...
    -- ใช้ header x-goog-api-key แทน ?key= (รองรับทั้ง AIza และ AQ.)
    ...
    if not respData.candidates or not respData.candidates[1] then
        return nil, "no candidates in resp"
    end
    local cand = respData.candidates[1]
    if not cand.content or not cand.content.parts or not cand.content.parts[1] then
...
-- ลองโมเดลทั้งหมดตามลำดับ (retry อัตโนมัติ)
local function askGemini()
    if not HARDCODED_KEY or HARDCODED_KEY == "" then
    ...
    -- เริ่มลองจาก model ปัจจุบัน
    local startIdx = currentModelIndex
    local totalModels = #GEMINI_MODELS

    for attempt = 0, totalModels - 1 do
        local idx = ((startIdx - 1 + attempt) % totalModels) + 1
        local model = GEMINI_MODELS[idx]

        addLine("[AI] trying " .. model .. "...", PURPLE_AI)
        local t0 = tick()
        local answer, err = callGemini(model, prompt)
        local dt = tick() - t0

        if answer then
            currentModelIndex = idx
            modelLbl.Text = "Model: " .. model
            addLine("[AI] ✓ " .. model .. " " .. string.format("%.1f", dt) .. "s", GREEN_OK)
            return answer, nil
        else
            addLine("[AI] ✗ " .. model .. ": " .. tostring(err), ORANGE_WARN)
            -- ถ้า 401 ครั้งแรก → ลอง model ถัดไป
            -- ถ้า 429 (quota) → ลอง model ถัดไป
            -- ถ้า 400/403/404 → ลอง model ถัดไป
        end
        task.wait(0.3)
    end
...
-- ==================== INIT ====================
addLine("🐷 v8.9 ready", GREEN_OK)
addLine("小女孩 | 🤖 | ▶ AUTO", PINK_DEEP)
addLine("Ctrl+1 = AI | Ctrl+2 = AUTO", PINK_DARK)
addLine("[KEY] embedded 🔑", GREEN_OK)
statusLbl.Text = "🐽 Ready."
updateLabels()

print("[ATLAS v8.9 🐷 Hardcoded Gemini] Loaded ✓")
