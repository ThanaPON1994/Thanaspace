-- =====================================================================
-- 🎨 DRAW AI V15.0 ULTIMATE — 10 TIER FILTER SYSTEM
-- 🔥 Full Integration: V14.7 GUI + 10 Tier Filter + Neural Scoring
-- =====================================================================
...
-- ========== 🎨 10 TIER STATE (V15.0) ==========
local drawMode = 5 -- ค่าเริ่มต้น: Tier 5 (Professional)
local tierNames = {
    "T1:Basic",
    "T2:Standard", 
    "T3:Advanced",
...
local tierColors = {
    Color3.fromRGB(100, 255, 100),   -- T1: เขียว
    Color3.fromRGB(150, 255, 50),    -- T2: เขียวเหลือง
    Color3.fromRGB(255, 255, 50),    -- T3: เหลือง
    Color3.fromRGB(255, 200, 50),    -- T4: ส้มอ่อน
    Color3.fromRGB(255, 150, 50),    -- T5: ส้ม
    Color3.fromRGB(255, 100, 50),    -- T6: ส้มแดง
    Color3.fromRGB(255, 50, 100),    -- T7: แดง
    Color3.fromRGB(200, 50, 255),    -- T8: ม่วง
    Color3.fromRGB(100, 50, 255),    -- T9: น้ำเงินม่วง
    Color3.fromRGB(50, 200, 255)     -- T10: ฟ้า
}
...
function calculatePointConsistency(strokes)
    if not strokes or #strokes == 0 then return 0 end
    local allDistances = {}
    
    for _, s in ipairs(strokes) do
        if #s >= 2 and #s[1] >= 2 then
            for i = 2, #s[1] do
                local dx = s[1][i] - s[1][i-1]
                local dy = s[2][i] - s[2][i-1]
                table.insert(allDistances, math.sqrt(dx*dx + dy*dy))
            end
        end
    end
    ...
    local sum = 0
    for _, d in ipairs(allDistances) do sum = sum + d end
    local mean = sum / #allDistances
    if mean == 0 then return 0 end
    
    local variance = 0
    for _, d in ipairs(allDistances) do variance = variance + (d - mean)^2 end
    local cv = math.sqrt(variance) / mean
    
    return math.max(0, 100 - cv * 100)
end

function calculateSpatialDistribution(strokes)
    if not strokes or #strokes == 0 then return 0 end
    local minX, maxX, minY, maxY = math.huge, -math.huge, math.huge, -math.huge
    local allPoints = {}
    ...
    for _, s in ipairs(strokes) do
        if #s >= 2 then
            for i = 1, #s[1] do
                local x, y = s[1][i], s[2][i]
                minX = math.min(minX, x); maxX = math.max(maxX, x)
                minY = math.min(minY, y); maxY = math.max(maxY, y)
                table.insert(allPoints, {x = x, y = y})
            end
        end
    end
    
    if #allPoints == 0 then return 0 end
    local w, h = maxX - minX, maxY - minY
    if w == 0 or h == 0 then return 0 end
    
    local gridSize = 4
    local grid = {}
    for i = 1, gridSize * gridSize do grid[i] = 0 end
    
    for _, p in ipairs(allPoints) do
        local gx = math.floor((p.x - minX) / w * gridSize)
        local gy = math.floor((p.y - minY) / h * gridSize)
        gx = math.min(gx, gridSize - 1)
        gy = math.min(gy, gridSize - 1)
        grid[gy * gridSize + gx + 1] = grid[gy * gridSize + gx + 1] + 1
    end
    
    local filledCells = 0
    for _, count in ipairs(grid) do
        if count > 0 then filledCells = filledCells + 1 end    end
    
    return (filledCells / (gridSize * gridSize)) * 100
end

function calculateCurvatureQuality(strokes)
    if not strokes or #strokes == 0 then return 0 end
    ...
    for _, s in ipairs(strokes) do
        if #s >= 2 and #s[1] >= 3 then
            for i = 2, #s[1] - 1 do
                ...
                local cross = vx1 * vy2 - vy1 * vx2
                local mag1 = math.sqrt(vx1^2 + vy1^2)
                local mag2 = math.sqrt(vx2^2 + vy2^2)
                
                if mag1 > 0 and mag2 > 0 then
                    totalCurvature = totalCurvature + math.abs(cross) / (mag1 * mag2)
                    totalSegments = totalSegments + 1
                end
            end
        end
    ...
    if totalSegments == 0 then return 0 end
    local avgCurvature = totalCurvature / totalSegments
    return math.min(100, avgCurvature * 200)
end

function calculateIntersectionScore(strokes)
    if not strokes or #strokes < 2 then return 0 end
    
    local function segmentsIntersect(p1, p2, p3, p4)
        local d1x = p2.x - p1.x; local d1y = p2.y - p1.y
        local d2x = p4.x - p3.x; local d2y = p4.y - p3.y
        
        local cross = d1x * d2y - d1y * d2x
        if math.abs(cross) < 0.001 then return false end
        
        local t = ((p3.x - p1.x) * d2y - (p3.y - p1.y) * d2x) / cross
        local u = ((p3.x - p1.x) * d1y - (p3.y - p1.y) * d1x) / cross
    ...
    for i = 1, #strokes - 1 do
        for j = i + 1, #strokes do
            if #strokes[i] >= 2 and #strokes[j] >= 2 then
                for a = 1, #strokes[i] - 1 do
                    for b = 1, #strokes[j] - 1 do
                        if segmentsIntersect(
                            strokes[i][a], strokes[i][a+1],
                            strokes[j][b], strokes[j][b+1]
                        ) then
    ...
    if totalChecks == 0 then return 50 end
    local ratio = intersections / totalChecks
    return math.min(100, math.max(0, ratio * 500))
end

function calculateComplexityScore(strokes)
    if not strokes or #strokes == 0 then return 0 end
    ...
    if totalPoints == 0 then return 0 end
    local complexity = totalLength / totalPoints
    
    if complexity < 1 then return complexity * 50
    elseif complexity < 5 then return 50 + (complexity - 1) * 12.5
    else return 100 end
end

function calculateSymmetryScore(strokes)
    if not strokes or #strokes == 0 then return 0 end
    local minX, maxX, minY, maxY = math.huge, -math.huge, math.huge, -math.huge
    local allPoints = {}
    ...
    for _, p in ipairs(allPoints) do
        if p.x < cx then
            table.insert(leftPoints, {x = (cx - p.x) / w, y = (p.y - minY) / (maxY - minY + 1)})
        else
            table.insert(rightPoints, {x = (p.x - cx) / w, y = (p.y - minY) / (maxY - minY + 1)})
        end
    end
    
    if #leftPoints < 5 or #rightPoints < 5 then return 30 end
    
    local ratio = math.min(#leftPoints, #rightPoints) / math.max(#leftPoints, #rightPoints)
    return math.min(100, ratio * 100)
end

function calculateMultiScaleScore(strokes)
    if not strokes or #strokes == 0 then return 0 end
    local scales = {0.5, 0.75, 1.0, 1.25, 1.5}
    local scores = {}
    ...
    for _, scale in ipairs(scales) do
        ...
        for _, s in ipairs(strokes) do
            if #s >= 2 then
                local newStroke = {}
                for i = 1, #s[1] do
                    table.insert(newStroke, {
                        x = s[1][i] * scale,
                        y = s[2][i] * scale
                    })
                end
                table.insert(scaledStrokes, newStroke)
            end
        end
        table.insert(scores, calculateStrokeSmoothness(scaledStrokes))
    end
    ...
    local variance = 0
    for _, s in ipairs(scores) do variance = variance + (s - mean)^2 end
    local stdDev = math.sqrt(variance)
    
    return math.max(0, 100 - stdDev * 2)
end

function calculateNeuralConfidence(strokes, theme)
    if not strokes or #strokes == 0 then return 0, {} end
    
    local metrics = {
        smoothness = calculateStrokeSmoothness(strokes),
        density = calculateStrokeDensity(strokes),
        aspect = calculateAspectBalance(strokes),
        consistency = calculatePointConsistency(strokes),
        distribution = calculateSpatialDistribution(strokes),
        curvature = calculateCurvatureQuality(strokes),
        intersection = calculateIntersectionScore(strokes),
        complexity = calculateComplexityScore(strokes),
        symmetry = calculateSymmetryScore(strokes),
        multiScale = calculateMultiScaleScore(strokes)
    }
...
function calculateCrossValidationScore(strokes, theme)
    local metrics = {
        smoothness = calculateStrokeSmoothness(strokes),
        density = calculateStrokeDensity(strokes),
        aspect = calculateAspectBalance(strokes),
    ...
    local cv = stdDev / (mean + 1)
    
    local crossVal = 100 - cv * 50
    
    if theme == "saw" or theme == "brain" then
        crossVal = crossVal * 1.15
    elseif theme == "elephant" or theme == "flamingo" then
        crossVal = crossVal * 1.1
    end
    
    return math.max(0, math.min(100, crossVal))
end

-- =====================================================================
-- 🎯 10 TIER FILTER FUNCTIONS
-- =====================================================================

-- TIER 1: BASIC
function isGoodDrawingTier1(theme, strokes, recognized)
    if recognized == false then return false end
    return validateStrokes(theme, strokes)
end

-- TIER 2: STANDARD
function isGoodDrawingTier2(theme, strokes, recognized)
    if not isGoodDrawingTier1(theme, strokes, recognized) then return false end
    ...
    if strokeCount < 2 then return false end
    if totalPoints < 20 then return false end
...
-- TIER 3: ADVANCED
function isGoodDrawingTier3(theme, strokes, recognized)
    if not isGoodDrawingTier2(theme, strokes, recognized) then return false end
    
    local smoothness = calculateStrokeSmoothness(strokes)
    local density = calculateStrokeDensity(strokes)
    
    if smoothness < 40 then return false end
    if density < 30 then return false end
    
    return true
end

-- TIER 4: EXPERT
function isGoodDrawingTier4(theme, strokes, recognized)
    if not isGoodDrawingTier3(theme, strokes, recognized) then return false end
    
    local aspect = calculateAspectBalance(strokes)
    local consistency = calculatePointConsistency(strokes)
    
    if aspect < 30 then return false end
    if consistency < 35 then return false end
    
    return true
end

-- TIER 5: PROFESSIONAL
function isGoodDrawingTier5(theme, strokes, recognized)
    if not isGoodDrawingTier4(theme, strokes, recognized) then return false end
    
    local distribution = calculateSpatialDistribution(strokes)
    local curvature = calculateCurvatureQuality(strokes)
    
    if distribution < 40 then return false end
    if curvature < 45 then return false end
    
    return true
end

-- TIER 6: ELITE
function isGoodDrawingTier6(theme, strokes, recognized)
    if not isGoodDrawingTier5(theme, strokes, recognized) then return false end
    
    local intersection = calculateIntersectionScore(strokes)
    local complexity = calculateComplexityScore(strokes)
    
    if intersection < 30 then return false end
    if complexity < 40 then return false end
    
    return true
end

-- TIER 7: MASTER
function isGoodDrawingTier7(theme, strokes, recognized)
    if not isGoodDrawingTier6(theme, strokes, recognized) then return false end
    
    local symmetry = calculateSymmetryScore(strokes)
    local multiScale = calculateMultiScaleScore(strokes)
    
    if symmetry < 25 then return false end
    if multiScale < 50 then return false end
    
    return true
end

-- TIER 8: GRANDMASTER
function isGoodDrawingTier8(theme, strokes, recognized)
    if not isGoodDrawingTier7(theme, strokes, recognized) then return false end
    
    local confidence, _ = calculateNeuralConfidence(strokes, theme)
    
    if confidence < 70 then return false end
    
    return true
end

-- TIER 9: LEGENDARY
function isGoodDrawingTier9(theme, strokes, recognized)
    if not isGoodDrawingTier8(theme, strokes, recognized) then return false end
    
    local confidence, _ = calculateNeuralConfidence(strokes, theme)
    local cvScore = calculateCrossValidationScore(strokes, theme)
    
    if confidence < 80 then return false end
    if cvScore < 75 then return false end
    
    return true
end

-- TIER 10: WORLD CLASS
function isGoodDrawingTier10(theme, strokes, recognized)
    if not isGoodDrawingTier9(theme, strokes, recognized) then return false end
    
    local confidence, metrics = calculateNeuralConfidence(strokes, theme)
    local cvScore = calculateCrossValidationScore(strokes, theme)
    
    if confidence < 90 then return false end
    if cvScore < 85 then return false end
...
function isGoodDrawingByTier(theme, strokes, recognized)
    if drawMode < 1 then drawMode = 1 end
    if drawMode > 10 then drawMode = 10 end
    return tierFunctions[drawMode](theme, strokes, recognized)
end

-- ========== SCAN LIMITS ==========
local scanLimits = {15, 15, 15, 15, 20, 30, 50, 75, 100, 200}

function getScanLimit()
    return scanLimits[drawMode] or 50
end

-- ========== TOTAL SCORE ==========
function calculateTotalScore(strokes, theme)
    local confidence, _ = calculateNeuralConfidence(strokes, theme)
    local cvScore = calculateCrossValidationScore(strokes, theme)
    local multiScale = calculateMultiScaleScore(strokes)
    return confidence * 0.5 + cvScore * 0.3 + multiScale * 0.2
end
...
function preloadPopularThemes()
    local popular = {"cat", "dog", "fish", "car", "house", "tree", "flower", "sun", "moon", "star", "heart", "book", "cup", "hat", "shoe", "bird", "apple", "banana", "cake", "chair"}
    log("📥 Preloading " .. #popular .. " themes...")
    local count = 0
    for _, theme in ipairs(popular) do
        local normalized = normalizeTheme(theme)
        if normalized and not apiCache[normalized] then
            getGoogleQuickDrawAsync(normalized, function(strokes)
                if strokes then count = count + 1 end
                if count >= #popular then preloadComplete = true; log("✅ Preload complete!") end
            end)
        end
    end
end
...
function getGoogleQuickDrawAsync(theme, callback)
    ...
    task.spawn(function()
        ...
        if res and #res > 10 then
            ...
            for i, obj in ipairs(objects) do
                ...
                if ok and data and data.drawing and #data.drawing > 0 then
                    ...
                    if validateStrokes(theme, validStrokes) then
                        ...
                        if isGoodDrawingByTier(theme, validStrokes, recognized) then
                            if drawMode >= 5 then
                                local score = calculateTotalScore(validStrokes, theme)
                                table.insert(candidates, {strokes = validStrokes, score = score})
                            else
                                finalStrokes = validStrokes
                                log("✅ " .. theme .. " | " .. tierNames[drawMode] .. " | ผ่านการกรอง")
                                break
                            end
                        end
                    end
            ...
            if drawMode >= 5 and #candidates > 0 then
                table.sort(candidates, function(a, b) return a.score > b.score end)
                finalStrokes = candidates[1].strokes
                log("🏆 " .. theme .. " | " .. tierNames[drawMode] .. " | Score: " .. math.floor(candidates[1].score) .. " (จาก " .. #candidates .. " ตัวเลือก)")
            end
        end
        
        if not finalStrokes then
            log("🔄 " .. theme .. " | API ล้มเหลว → Fallback")
...
function drawTheme(theme, forceSync)
    ...
    log("🎯 Drawing: " .. theme .. " → API: " .. normalized .. " | Tier: " .. tierNames[drawMode])
...
local tLabel = Instance.new("TextLabel"); tLabel.Size = UDim2.new(1, 0, 0, 24); tLabel.BackgroundColor3 = Color3.fromRGB(0, 255, 100); tLabel.Text = "🧠 V15.0 — 10 TIER FILTER"; tLabel.TextColor3 = Color3.fromRGB(0, 0, 0); tLabel.TextSize = 10; tLabel.Font = Enum.Font.GothamBold; tLabel.Parent = main
...
local modeLabel = Instance.new("TextLabel"); modeLabel.Size = UDim2.new(0.9, 0, 0, 14); modeLabel.Position = UDim2.new(0.05, 0, 0.265, 0); modeLabel.BackgroundColor3 = Color3.fromRGB(15, 5, 30); modeLabel.Text = "🎨 Tier: T5:Professional"; modeLabel.TextColor3 = tierColors[5]; modeLabel.TextSize = 7; modeLabel.Font = Enum.Font.Code; modeLabel.Parent = main
...
forceBtn.MouseButton1Click:Connect(function()
    if currentTheme then forceBtn.BackgroundColor3 = Color3.fromRGB(255, 255, 0); local normalized = normalizeTheme(currentTheme); apiCache[normalized] = nil; log("🔄 FORCE DRAW: " .. tierNames[drawMode]); drawTheme(currentTheme, true); task.wait(0.5); forceBtn.BackgroundColor3 = Color3.fromRGB(100, 150, 255) end
end)

-- SIZE
local sizeDown = Instance.new("TextButton"); sizeDown.Size = UDim2.new(0.12, 0, 0, 28); sizeDown.Position = UDim2.new(0.04, 0, 0.56, 0); sizeDown.BackgroundColor3 = Color3.fromRGB(255, 100, 100); sizeDown.Text = "−"; sizeDown.TextColor3 = Color3.fromRGB(255, 255, 255); sizeDown.TextSize = 16; sizeDown.Font = Enum.Font.GothamBold; sizeDown.Parent = main
local sizeLabel = Instance.new("TextLabel"); sizeLabel.Size = UDim2.new(0.14, 0, 0, 28); sizeLabel.Position = UDim2.new(0.17, 0, 0.56, 0); sizeLabel.BackgroundColor3 = Color3.fromRGB(20, 20, 20); sizeLabel.Text = "35%"; sizeLabel.TextColor3 = Color3.fromRGB(0, 255, 100); sizeLabel.TextSize = 10; sizeLabel.Font = Enum.Font.Code; sizeLabel.Parent = main
local sizeUp = Instance.new("TextButton"); sizeUp.Size = UDim2.new(0.12, 0, 0, 28); sizeUp.Position = UDim2.new(0.32, 0, 0.56, 0); sizeUp.BackgroundColor3 = Color3.fromRGB(100, 255, 100); sizeUp.Text = "+"; sizeUp.TextColor3 = Color3.fromRGB(0, 0, 0); sizeUp.TextSize = 16; sizeUp.Font = Enum.Font.GothamBold; sizeUp.Parent = main
...
-- SKIP
local skipDown = Instance.new("TextButton"); skipDown.Size = UDim2.new(0.12, 0, 0, 28); skipDown.Position = UDim2.new(0.47, 0, 0.56, 0); skipDown.BackgroundColor3 = Color3.fromRGB(255, 100, 100); skipDown.Text = "−"; skipDown.TextColor3 = Color3.fromRGB(255, 255, 255); skipDown.TextSize = 16; skipDown.Font = Enum.Font.GothamBold; skipDown.Parent = main
local skipCountLabel = Instance.new("TextLabel"); skipCountLabel.Size = UDim2.new(0.14, 0, 0, 28); skipCountLabel.Position = UDim2.new(0.60, 0, 0.56, 0); skipCountLabel.BackgroundColor3 = Color3.fromRGB(20, 20, 20); skipCountLabel.Text = "5"; skipCountLabel.TextColor3 = Color3.fromRGB(0, 255, 255); skipCountLabel.TextSize = 10; skipCountLabel.Font = Enum.Font.Code; skipCountLabel.Parent = main
local skipUp = Instance.new("TextButton"); skipUp.Size = UDim2.new(0.12, 0, 0, 28); skipUp.Position = UDim2.new(0.75, 0, 0.56, 0); skipUp.BackgroundColor3 = Color3.fromRGB(100, 255, 100); skipUp.Text = "+"; skipUp.TextColor3 = Color3.fromRGB(0, 0, 0); skipUp.TextSize = 16; skipUp.Font = Enum.Font.GothamBold; skipUp.Parent = main
...
-- 🎨 V15.0: 10 TIER BUTTONS
local tierBtns = {}
local tierStartY = 0.625
local tierBtnH = 0.023
local tierGap = 0.003
...
for i = 1, 10 do
    ...
    btn.Size = UDim2.new(0.43, 0, 0, 20)
    btn.Position = UDim2.new(0.04 + col * 0.49, 0, tierStartY + row * (tierBtnH + tierGap), 0)
    btn.Text = tierNames[i]
    btn.TextColor3 = (i == drawMode) and Color3.fromRGB(0, 0, 0) or Color3.fromRGB(255, 255, 255)
    btn.BackgroundColor3 = (i == drawMode) and tierColors[i] or Color3.fromRGB(40, 40, 60)
    btn.TextSize = 7
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        updateTierButtons()
        modeLabel.Text = "🎨 Tier: " .. tierNames[drawMode]
        modeLabel.TextColor3 = tierColors[drawMode]
        log("🎯 " .. tierNames[drawMode] .. " | Cache cleared | Scan: " .. getScanLimit())
    end)
...
function updateTierButtons()
    for i = 1, 10 do
        tierBtns[i].BackgroundColor3 = (i == drawMode) and tierColors[i] or Color3.fromRGB(40, 40, 60)
        tierBtns[i].TextColor3 = (i == drawMode) and Color3.fromRGB(0, 0, 0) or Color3.fromRGB(255, 255, 255)
    end
end
...
-- PRELOAD
local preloadBtn = Instance.new("TextButton"); preloadBtn.Size = UDim2.new(0.43, 0, 0, 28); preloadBtn.Position = UDim2.new(0.04, 0, 0.82, 0); preloadBtn.BackgroundColor3 = Color3.fromRGB(200, 100, 255); preloadBtn.Text = "📥 PRELOAD"; preloadBtn.TextColor3 = Color3.fromRGB(0, 0, 0); preloadBtn.TextSize = 8; preloadBtn.Font = Enum.Font.GothamBold; preloadBtn.Parent = main; Instance.new("UICorner", preloadBtn).CornerRadius = UDim.new(0, 4)
preloadBtn.MouseButton1Click:Connect(function() preloadPopularThemes() end)

-- CLEAR
local clearBtn = Instance.new("TextButton"); clearBtn.Size = UDim2.new(0.43, 0, 0, 28); clearBtn.Position = UDim2.new(0.53, 0, 0.82, 0); clearBtn.BackgroundColor3 = Color3.fromRGB(255, 80, 80); clearBtn.Text = "🗑️ Clear Cache"; clearBtn.TextColor3 = Color3.fromRGB(255, 255, 255); clearBtn.TextSize = 8; clearBtn.Font = Enum.Font.GothamBold; clearBtn.Parent = main; Instance.new("UICorner", clearBtn).CornerRadius = UDim.new(0, 4)
...
log("🚀 DRAW AI V15.0 — 10 TIER FILTER LOADED!")
log("🎯 Tier 1-10: Basic → World Class")
log("🔍 Scan Limits: T1-4=15, T5=20, T6=30, T7=50, T8=75, T9=100, T10=200")
