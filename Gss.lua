-- ATLAS v10.1 — AUTO v1.4 FAST SMART + COPY LOG ⚡
-- 🐷 Pink Pig Edition (350 x 300) — 傳說中的龍女來了
-- 🆕 Substring + Category + Country + Backtrack
...
local function copyToClipboard(text)
    ...
    if Clipboard and Clipboard.set then
        local ok = pcall(function() Clipboard.set(text) end); if ok then return true end
    end
...
local function addPathLog(page, src)
    ...
    table.insert(hopHistory, page)
    if #hopHistory > 8 then table.remove(hopHistory, 1) end
end
...
local function showLogPopup(text)
    ...
    box.ClearTextOnFocus = false; box.Text = text
    box.BorderSizePixel = 0; box.ZIndex = 101; box.Parent = logPopup
    Instance.new("UICorner", box).CornerRadius = UDim.new(0, 8)

    close.MouseButton1Click:Connect(function()
        logPopup:Destroy(); logPopup = nil
    end)
end

local function formatPathLog()
    local lines = {}
    table.insert(lines, "=== ATLAS v10.1 PATH LOG ===")
...
local function copyPathLog()
    ...
    if ok then
        ...
        addLine("[COPY] ✗ copy ไม่ได้ — popup", ORANGE_WARN)
        statusLbl.Text = "📋 Copy failed"
    end
    showLogPopup(text)
end
            ...
            if (isFrame or isImage) and isVisibleChain(obj) then
                ...
                if sizeOk and squareOk then
                    ...
                    if isRealCircle then
                        ...
                        if isFrame and obj.BackgroundTransparency < 0.3 and isRedColor(obj.BackgroundColor3) then
                            red = true; reason = "bg"
                        end
                        if not red and isImage and obj.ImageTransparency < 0.3 and isRedColor(obj.ImageColor3) then
                            red = true; reason = "img"
                        end
                        if not red then
                            local stroke = obj:FindFirstChildOfClass("UIStroke")
                            if stroke and stroke.Transparency < 0.3 and isRedColor(stroke.Color) then
...
local function startInstantChain(picks)
    ...
    for _, p in ipairs(picks) do
        ...
        if t ~= "" then
            ...
            if r > chainCurrentRank then table.insert(autoChainQueue, t) else skippedCount = skippedCount + 1 end
        end
    end
    if #autoChainQueue == 0 then
        addLine("[CHAIN] ไม่มี rank ถัดไป", ORANGE_WARN); return
    end
...
-- ============================================================
-- 🎯 SMART SCORING v1.4 (FAST — ไม่ใช้ AI)
-- ============================================================
local STOPWORDS = {
...
-- 🎯 TARGET_HINTS v1.4 — 200+ entries
local TARGET_HINTS = {
    -- 🎮 VIDEO GAMES
    ["silent hill"] = {"horror","survival","konami","playstation","fog","monster","game","sequel"},
    ["resident evil"] = {"horror","survival","capcom","playstation","zombie","game"},
    ["final fantasy"] = {"rpg","square","enix","playstation","fantasy","game"},
    ["mario"] = {"nintendo","platformer","game","jump","plumber","luigi"},
    ["zelda"] = {"nintendo","adventure","game","link","hyrule","fantasy"},
    ["minecraft"] = {"sandbox","mojang","game","block","survival","craft"},
    ["pokemon"] = {"nintendo","game","monster","trainer","pikachu","creature"},
    ["halo"] = {"xbox","microsoft","shooter","game","master","spartan"},
    ["grand theft auto"] = {"rockstar","game","playstation","crime","sandbox"},
    ["call of duty"] = {"shooter","war","game","activision","military"},
    ["warcraft"] = {"blizzard","strategy","game","rpg","online"},
    ["doom"] = {"shooter","game","id software","demon","mars"},
    ["terraria"] = {"sandbox","game","2d","adventure","survival","re-logic"},
    ["undertale"] = {"rpg","game","indie","toby fox"},
    ["hollow knight"] = {"game","indie","metroidvania"},
    ["game"] = {"video game","playstation","nintendo","xbox","pc","console","gameplay","arcade","sega","atari"},
    ["video game"] = {"playstation","nintendo","xbox","pc","console","gameplay","arcade","sega","atari"},
    -- 🍔 FOOD
    ["hamburger"] = {"hamburg","german","germany","food","beef","bun","sandwich","cuisine"},
    ["pizza"] = {"italian","italy","food","cheese","tomato","naples","cuisine"},
    ["sushi"] = {"japanese","japan","food","fish","rice","cuisine"},
    ["ramen"] = {"japanese","japan","noodle","soup","food","cuisine"},
    ["taco"] = {"mexican","mexico","food","tortilla","cuisine"},
    ["baguette"] = {"french","france","bread","food","cuisine"},
    ["curry"] = {"indian","india","spice","food","dish"},
    ["bread"] = {"food","wheat","bake","flour","loaf"},
    ["cheese"] = {"food","dairy","milk","italian","french"},
    -- 🏙️ CITIES
    ["berlin"] = {"germany","german","capital","city","europe"},
    ["paris"] = {"france","french","capital","city","europe"},
    ["london"] = {"england","english","capital","city","united kingdom","british"},
    ["tokyo"] = {"japan","japanese","capital","city","asia"},
    ["rome"] = {"italy","italian","capital","city","roman"},
    ["madrid"] = {"spain","spanish","capital","city"},
    ["moscow"] = {"russia","russian","capital","city"},
    ["hamburg"] = {"germany","german","city","port","harbor"},
    ["new york"] = {"united states","american","city","manhattan"},
    -- 👤 PEOPLE
    ["hogan"] = {"wrestler","wrestling","actor","american","wwf","wwe","terry","bollea","entertainer"},
    ["einstein"] = {"physicist","physics","scientist","german","nobel","relativity","theory"},
    ["napoleon"] = {"emperor","french","france","general","bonaparte","war"},
    ["mozart"] = {"composer","austrian","music","classical","opera","symphony"},
    ["tesla"] = {"inventor","serbian","engineer","physics","electricity","alternating","coil"},
    ["newton"] = {"physicist","physics","mathematician","english","gravity","principle","motion"},
    ["darwin"] = {"naturalist","biologist","english","evolution","species","origin"},
    ["shakespeare"] = {"playwright","english","poet","drama","theatre","sonnet","hamlet"},
    ["trump"] = {"president","american","politics","republican","business"},
    ["obama"] = {"president","american","politics","democrat"},
    ["lincoln"] = {"president","american","politics","civil war","republican"},
    -- 🎬 MOVIES
    ["star wars"] = {"lucas","film","movie","sci-fi","jedi","skywalker"},
    ["harry potter"] = {"rowling","fantasy","book","film","wizard","hogwarts"},
    ["lord of the rings"] = {"tolkien","fantasy","book","film","hobbit"},
    ["matrix"] = {"sci-fi","film","movie","keanu","reeves"},
    ["titanic"] = {"film","movie","ship","iceberg","dicaprio"},
    ["avatar"] = {"cameron","film","movie","pandora","sci-fi"},
    -- 🌍 PLACES
    ["pole"] = {"antarctica","arctic","geography","latitude","ice","polar"},
    -- 🔬 SCIENCE
    ["climate"] = {"weather","temperature","environment","atmosphere","rain","wind"},
    ...
    -- 💰 POLITICS/ECONOMY
    ["war"] = {"military","battle","army","soldier","conflict","history"},
    ["economy"] = {"economic","trade","market","finance","bank"},
    ["religion"] = {"christianity","islam","buddhism","hinduism","god","church"},
    -- 🎵 MUSIC
    ["music"] = {"song","album","band","singer","composer","instrument"},
    ["beatles"] = {"music","band","liverpool","english","rock"},
    -- 🎨 CULTURE
    ["art"] = {"painting","artist","museum","culture","renaissance"},
    ["film"] = {"movie","actor","actress","director","cinema","hollywood"},
    ["history"] = {"war","empire","ancient","century","civilization","king"},
    ["city"] = {"country","state","population","capital","urban","town"},
}

-- 🆕 CATEGORY KEYWORDS
local CATEGORY_KEYWORDS = {
    food = {"food","cuisine","dish","cooking","restaurant","eat","meal","bread","meat","cheese","sandwich","recipe"},
    city = {"city","capital","urban","population","metropolis","town","municipality"},
    game = {"video game","playstation","nintendo","xbox","pc game","console","gameplay","arcade","sega"},
    person = {"born","biography","died","career","actor","singer","politician","author"},
    country = {"country","nation","republic","government","sovereign","state"},
    animal = {"animal","species","mammal","biology","zoology","wildlife"},
    plant = {"plant","flower","tree","botany","species","leaf"},
...
local TARGET_CATEGORY = {
    ["hamburger"]="food", ["pizza"]="food", ["sushi"]="food", ["bread"]="food",
    ["cheese"]="food", ["taco"]="food", ["ramen"]="food",
    ["berlin"]="city", ["paris"]="city", ["tokyo"]="city", ["london"]="city",
    ["rome"]="city", ["moscow"]="city", ["hamburg"]="city", ["new york"]="city",
    ["silent hill"]="game", ["mario"]="game", ["zelda"]="game", ["pokemon"]="game",
    ["minecraft"]="game", ["halo"]="game", ["terraria"]="game",
    ["einstein"]="person", ["napoleon"]="person", ["mozart"]="person",
    ["tesla"]="person", ["newton"]="person", ["darwin"]="person",
}

-- 🆕 TARGET → COUNTRY (food/city origin)
local TARGET_COUNTRY = {
    ["hamburger"]="german", ["pizza"]="italian", ["sushi"]="japanese",
    ["ramen"]="japanese", ["taco"]="mexican", ["baguette"]="french",
    ["curry"]="indian", ["croissant"]="french",
    ["berlin"]="german", ["paris"]="french", ["tokyo"]="japanese",
    ["london"]="english", ["rome"]="italian", ["moscow"]="russian",
    ["hamburg"]="german",
    ["einstein"]="german", ["mozart"]="austrian", ["napoleon"]="french",
}

local HUB_WORDS = {"history","war","science","university","country","city","language","culture","people","world","national"}

local function getTargetWords(tLow)
    local words = {}
...
local function getTargetCategory(tLow)
    for key, cat in pairs(TARGET_CATEGORY) do
        if tLow:find(key, 1, true) then return cat end
    end
    -- auto-detect
    if tLow:find("game") or tLow:find("mario") or tLow:find("zelda") then return "game" end
    if tLow:find("food") or tLow:find("cuisine") then return "food" end
    if tLow:find("city") or tLow:find("capital") then return "city" end
    return nil
end

local function getTargetCountry(tLow)
    for key, country in pairs(TARGET_COUNTRY) do
...
-- 🆕 v1.4 SCORE
local function scoreV14(candidate, targetLower, targetSup, targetWords, targetCat, targetCountry)
    ...
    -- 🎯 L1 SUBSTRING (แรงสุด — KEY ของ v1.4)
    if #cSup >= 4 and #targetSup >= 4 then
        if targetSup:find(cSup, 1, true) then return 200000 end   -- link ⊂ target
        if cSup:find(targetSup, 1, true) then return 150000 end   -- target ⊂ link
    end

    -- 🎯 L2 EXACT
    if targetSup ~= "" and cSup == targetSup then return 1000000 end

    local score = 0

    -- 🧠 L3 SHARED WORDS
    if targetLower ~= "" and targetWords then
        for w in pairs(targetWords) do
    ...
    -- 🎯 L4 TARGET HINTS
    if targetLower ~= "" then
        for key, hints in pairs(TARGET_HINTS) do
            if targetLower:find(key, 1, true) then
    ...
    -- 🆕 L5 CATEGORY MATCH
    if targetCat and CATEGORY_KEYWORDS[targetCat] then
        for _, w in ipairs(CATEGORY_KEYWORDS[targetCat]) do
            if cLow:find(w, 1, true) then score = score + 300 end
        end
    end

    -- 🆕 L6 COUNTRY MATCH
    if targetCountry and cLow:find(targetCountry, 1, true) then
        score = score + 400
    end

    -- 🆕 L7 HUB
    for _, w in ipairs(HUB_WORDS) do
        if cLow == w then score = score + 80
        elseif cLow:find("^" .. w .. " ") then score = score + 60
        elseif cLow:find(" " .. w .. "$") then score = score + 60
        end
    end
...
-- 🆕 v1.4: ตรวจ backtrack
local function isBacktrack(c)
    local cl = c:lower()
    for i = math.max(1, #hopHistory - 4), #hopHistory do
        if hopHistory[i] and hopHistory[i]:lower() == cl then return true end
    end
    return false
end

local function smartPick(pool, targetLower, targetSup, targetWords, targetCat, targetCountry)
    if #pool == 0 then return nil, "empty" end
    local scored = {}
    for _, c in ipairs(pool) do
        local s = scoreV14(c, targetLower, targetSup, targetWords, targetCat, targetCountry)
        if isBacktrack(c) then s = s - 10000 end
        table.insert(scored, {title=c, score=s})
    end
    table.sort(scored, function(a, b) return a.score > b.score end)

    -- 🎯 ถ้า link ⊂ target หรือ target ⊂ link → ยิงเลย
...
local function doAutoExploreStep()
    ...
    if not currentArticle or not currentArticleId then
        ...
        task.defer(function()
            if autoExploreActive then task.wait(0.15); doAutoExploreStep() end
        end)
        return
    end
    autoRetryCount = 0
    ...
    if #all == 0 then
        autoExploreBusy = false
        task.defer(function()
            if autoExploreActive then task.wait(0.3); doAutoExploreStep() end
        end)
        return
    end
    ...
    local tWords = getTargetWords(tLower)
    local tCat = getTargetCategory(tLower)
    local tCountry = getTargetCountry(tLower)

    local pick, kind = smartPick(pool, tLower, tSup, tWords, tCat, tCountry)
    if not pick then autoExploreBusy = false; return end

    addPathLog(pick, kind)
    local icon = kind == "SUBSTRING" and "💥" or kind == "top" and "🎯" or "🎲"
    local col = kind == "SUBSTRING" and PURPLE_HIT or kind == "top" and PINK_DEEP or ORANGE_WARN
    ...
    if not ok then
        task.defer(function()
            if autoExploreActive then task.wait(0.05); doAutoExploreStep() end
        end)
    end
end
...
local function startAutoExplore()
    ...
    addLine("[AUTO] 🚀 v1.4 SMART (substring+cat+country)", GREEN_OK)
...
pasteBtn.MouseButton1Click:Connect(function()
    ...
    task.wait(0.2); isProcessing = false
end)
...
goBtn.MouseButton1Click:Connect(function()
    if isProcessing then return end
    isProcessing = true; doGo(); task.wait(0.2); isProcessing = false
end)
...
addLine("⚡ v10.1 AUTO v1.4 FAST", GREEN_OK)
addLine("substring+category+country", PINK_DEEP)
addLine("85% top-3 / 15% explore", PINK_DARK)
statusLbl.Text = "🐽 Ready ⚡"
updateLabels()

print("[ATLAS v10.1 ⚡ AUTO v1.4 FAST] Loaded ✓")
