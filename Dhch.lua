-- ATLAS v9.4.3 — FULL FIX + FAST 777 + NO GOAL + NO-ID FALLBACK + NORMALIZED SEARCH
-- 🐷 Pink Pig Edition — Cap hidden 250
...
local function navigateToTitle(title)
    ...
    -- 🆕 ลองหาแบบไม่มีช่องว่าง (Christina Ricci → ChristinaRicci)
    if not targetId then
    ...
    if not targetId then
        ...
        -- 🆕 เก็บ Fallback ไว้ให้ปุ่ม 111
        lastFallbackCandidates = {}
        lastFallbackPage = currentArticle
        lastFallbackTarget = targetTitle
        for k, _ in pairs(linkIdMap) do
            table.insert(lastFallbackCandidates, k)
        end
        addLine("  [fallback] " .. #lastFallbackCandidates .. " links", ORANGE_WARN)
...
local function parsePicksFromText(text)
    ...
    -- 🆕 Fast-path: ตรวจ 2 บรรทัดก่อน (0 ms)
    ...
    -- 🆕 Fast-path 2: 1 บรรทัดเดียว → ใช้เลย
...
local function doGo()
    ...
    addLine("[check] linkIdMap = " .. idCount .. " ids", ORANGE_WARN)
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    -- 🆕 Prompt สั้น (CURRENT + Target : + รายการ)
    local lines = {
    ...
    addLine("[hop-src] cache=" .. cacheCount .. " | total=" .. #filtered, PINK_DARK)
...
copyLogBtn.MouseButton1Click:Connect(function()
    ...
    local ok = copyToClipboard(table.concat(lines, "\n"))
    addLine("[111] " .. #lastFallbackCandidates .. " fallback " .. (ok and "copied" or "FAIL"),
        ok and GREEN_OK or RED_ERR)
...
addLine("🐷 v9.4.3 FULL FIX + FAST 777", GREEN_OK)
addLine("小女孩 → Prompt สั้น | 777 → เร็วสุด", PINK_DEEP)
addLine("111 → Copy Fallback", ORANGE_WARN)
statusLbl.Text = "🐽 Ready"
updateLabels()

print("[ATLAS v9.4.3 🐷 FULL FIX] Loaded ✓")
